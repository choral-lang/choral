...
public class SystemDemo extends AbstractBehavior<SystemDemo.AuthSession> {
  ...
  private static final ActorSystem<SystemDemo.AuthSession> system =
      ActorSystem.create(SystemDemo.create(), "distauth_session");
  ...
  private Behavior<AuthSession> onStart(AuthSessionStart session) {
    ...
    try {
      ...
      ActorRef<Message> service =
          getContext().spawn(Service.create(getContext().getSelf()), "service");
      getContext()
          .spawn(
              Client.create(getContext().getSelf(), ip, service, new Credentials("john", "doe")),
              "client");
    } catch (Exception e) {
      e.printStackTrace();
    }
