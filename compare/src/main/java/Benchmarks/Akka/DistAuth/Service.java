...
import Benchmarks.Akka.DistAuth.Messages.AuthResultMessage_B;
import Benchmarks.Akka.DistAuth.Messages.Message;
...
public class Service extends AbstractBehavior<Message> {
  ...
  public Service(ActorContext<Message> context, ActorRef<SystemDemo.AuthSession> system) {
    super(context);
    this.system = system;
  }
  ...
  public static Behavior<Message> create(ActorRef<SystemDemo.AuthSession> system) {
    return Behaviors.setup(context -> new Service(context, system));
  }

  @Override
  public Receive<Message> createReceive() {
    return newReceiveBuilder().onMessage(AuthResultMessage_B.class, this::onMessage).build();
  }
