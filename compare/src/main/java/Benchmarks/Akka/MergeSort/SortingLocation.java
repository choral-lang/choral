...
public class SortingLocation extends AbstractBehavior<Message> {
  ...
  private Behavior<Message> onSort(SortMessage message) {
    ...
    if (l.size() > 2) {
      int pivot = l.size() / 2;
      getContext()
          .spawn(LeftSortingLocation.create(), "Location_B")
          .tell(new SortMessage(l.subList(0, pivot), getContext().getSelf()));
      getContext()
          .spawn(RightSortingLocation.create(), "Location_C")
          .tell(new SortMessage(l.subList(pivot, l.size()), getContext().getSelf()));
    } else {
      if (l.size() < 2) {
        ...
        message
            .merger()
            .tell(getSpecificMergeMessage(l.get(0) > l.get(1) ? List.of(l.get(1), l.get(0)) : l));
      }
    }
    return this;
  }
