 ...
 * choral.channels, when it is not possible to complete an operation, no zero is returned, but an
 * {@link WouldBlockException}. This divergence from the base interface is needed because both a
 * <code>read</code> and a <code>write</code> operation can run out of both bytesProduced for
 * reading and buffer space for writing, as a handshake (a bidirectional operation) can happen at
 * any moment. The user would use a {@link Selector} to wait for the expected condition <em>of the
 * underlying channel</em>, and should know which operation to register.
 *
...
public interface TlsChannel extends ByteChannel, GatheringByteChannel, ScatteringByteChannel {
   ...
   *     server-side choral.channels before the SNI is parsed).
   */
   ...
   * the number of bytesProduced remaining in the buffer, that is, <tt>dst.remaining()</tt>, at the
   * moment this method is invoked.
   *
   * <p>Suppose that a byte sequence of length <i>n</i> is read, where <tt>0</tt>
   * &nbsp;<tt>&lt;=</tt>&nbsp;<i>n</i>&nbsp;<tt>&lt;=</tt>&nbsp;<i>r</i>. This byte sequence will
   * be transferred into the buffer so that the first byte in the sequence is at index <i>p</i> and
   * the last byte is at index
   * <i>p</i>&nbsp;<tt>+</tt>&nbsp;<i>n</i>&nbsp;<tt>-</tt>&nbsp;<tt>1</tt>, where <i>p</i> is the
   * buffer's position at the moment this method is invoked. Upon return the buffer's position will
   * be equal to <i>p</i> &nbsp;<tt>+</tt>&nbsp;<i>n</i>; its limit will not have changed.
   *
   ...
   * @return The number of bytesProduced read, or <tt>-1</tt> if the channel has reached
   ...
   * the number of bytesProduced remaining in the buffer, that is, <tt>src.remaining()</tt>, at the
   * moment this method is invoked.
   *
   * <p>Suppose that a byte sequence of length <i>n</i> is written, where
   * <tt>0</tt>&nbsp;<tt>&lt;=</tt>&nbsp;<i>n</i>&nbsp;<tt>&lt;=</tt>&nbsp; <i>r</i>. This byte
   * sequence will be transferred from the buffer starting at index <i>p</i>, where <i>p</i> is the
   * buffer's position at the moment this method is invoked; the index of the last byte written will
   * be <i>p</i>&nbsp;<tt>+</tt>&nbsp;<i>n</i>&nbsp;<tt>-</tt>&nbsp;<tt>1</tt>. Upon return the
   * buffer's position will be equal to <i>p</i>&nbsp; <tt>+</tt>&nbsp;<i>n</i>; its limit will not
   * have changed.
   *
   ...
   *
   * <p>An invocation of this method of the form <tt>c.write(srcs)</tt> behaves in exactly the same
   ...
   *     are to be transferred; must be non-negative and no larger than <tt>dsts.length</tt>
   * @param length The maximum number of buffers to be accessed; must be non-negative and no larger
   ...
   *
   * <p>An invocation of this method of the form <tt>c.read(dsts)</tt> behaves in exactly the same
   * manner as the invocation
   *
   * <blockquote>
   *
