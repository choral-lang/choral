...
public class TlsChannelImpl implements ByteChannel {
  ...
  private SSLEngineResult callEngineWrap(ByteBufferSet source) throws SSLException {
    try {
      ...
      //			if (logger.isTraceEnabled()) {
      //				logger.trace("engine.wrap() result: [{}]; engine status: {}; srcBuffer: {},
      // outEncrypted: {}",
  ...
  private void ensureInPlainCapacity(int newCapacity) {
    if (inPlain.buffer.capacity() < newCapacity) {
      //			logger.trace("inPlain buffer too small, increasing from {} to {}",
      // inPlain.buffer.capacity(), newCapacity);
    ...
    try {
      if (force || !negotiated) {
        ...
        try {
          ...
          //                    logger.trace("client code threw exception in session initialization
          // callback", e);
      ...
      try {
        if (!shutdownSent) {
          try {
            ...
            //						logger.debug("error doing TLS shutdown on close(), continuing: {}",
            // e.getMessage());
          }
        }
      } finally {
