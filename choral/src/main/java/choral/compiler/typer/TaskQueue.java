...
/**
 * A priority queue of typechecking tasks, sorted by @Typer.Phase. We use the queue to ensure that
 * tasks in phase N are all completed before those in phase N+1.
 */
public class TaskQueue {
  ...
  public static class Task implements Comparable<Task>, Runnable {
    ...
    /**
     * Represents how many times isReady() was called when the task was not ready to be run. Is used
     * to impose natural ordering on tasks with same phase
     */
    int rounds = 0;
    ...
    protected boolean isReady() {
      if (!dependenciesReady) {
        if (type instanceof HigherTypeParameter) {
          dependenciesReady =
              ((HigherTypeParameter) type)
                  .innerType()
                  .upperBound()
                  .allMatch(GroundReferenceType::isInterfaceFinalised);
        } else {
          dependenciesReady =
              ((HigherClassOrInterface) type)
                  .innerType()
  ...
  /**
   * Enqueues the given task, silently dropping it if the status of the task is not WAITING or
   * READY.
   */
