...
public interface VariableDeclarationScope extends Scope {
  ...
  default GroundDataType assertLookupVariable(String identifier) {
    return lookupVariable(identifier).orElseThrow(() -> new UnresolvedSymbolException(identifier));
  }

  default GroundDataType assertLookupVariable(Name query) {
    return lookupVariable(query.identifier())
        .orElseThrow(
            () ->
  ...
  default GroundDataType assertLookupVariableOrField(String identifier) {
    return lookupVariableOrField(identifier)
        .orElseThrow(() -> new UnresolvedSymbolException(identifier));
  }

  default GroundDataType assertLookupVariableOrField(Name query) {
    return lookupVariableOrField(query.identifier())
        .orElseThrow(
            () ->
                new AstPositionedException(
                    query.position(), new UnresolvedSymbolException(query.identifier())));
  ...
  /**
   * Collects channels available in the scope by looking at the fields of "this" and the enclosing
   * method's arguments
   */
  default List<Pair<String, GroundInterface>> getChannels() {
    Map<String, GroundInterface> channels = new HashMap<>();
    HigherClassOrInterface diDataChannel =
        assertLookupClassOrInterface("choral.channels.DiDataChannel");
    HigherClassOrInterface diSelectChannel =
        assertLookupClassOrInterface("choral.channels.DiSelectChannel");
