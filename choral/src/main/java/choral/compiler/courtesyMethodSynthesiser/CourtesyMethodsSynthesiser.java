...
public class CourtesyMethodsSynthesiser extends AbstractChoralVisitor<Node> {
  ...
  public static CompilationUnit visitCompilationUnit(CompilationUnit n) {
    return new CompilationUnit(
        ...
        n.imports(),
        n.interfaces().stream()
            .map(CourtesyMethodsSynthesiser::visitInterface)
            .collect(Collectors.toList()),
        n.classes().stream()
            .map(CourtesyMethodsSynthesiser::visitClass)
            .collect(Collectors.toList()),
        n.enums(),
        "");
  }
  ...
  private static Class visitClass(Class n) {
    ...
    return new Class(
        ...
        n.typeParameters(),
        n.extendsClass(),
        n.implementsInterfaces(),
        v.visitAndCollect(n.fields()),
        methods,
        constructors,
        n.annotations(),
        n.modifiers(),
  ...
  public ClassMethodDefinition visit(ClassMethodDefinition n) {
    ...
    if (!unitParameters.isEmpty() && !n.modifiers().contains(ClassMethodModifier.PRIVATE)) {
      MethodSignature sytheticMethodSignature =
          new MethodSignature(
              n.signature().name(),
              ...
              n.position());
      List<Expression> parameterBypass =
          n.signature().parameters().stream()
              .filter(
                  p -> !unitParameters.contains(p.name())) // we keep only the non-Unit parameters
              .map(p -> new FieldAccessExpression(p.name()))
              .collect(Collectors.toList());
      List<TypeExpression> typeParameters =
          n.signature().typeParameters().stream()
              .map(
                  ftp ->
                      new TypeExpression(
                          ftp.name(),
      ...
      syntheticMethods.add(
          new ClassMethodDefinition(
              ...
              n.modifiers(),
              n.position()));
      MethodCallExpression proxyMethod =
          new MethodCallExpression(n.signature().name(), parameterBypass, typeParameters);
              ...
              .map(
                  ftp ->
                      new TypeExpression(
                          ...
                          Collections.emptyList()))
              .collect(Collectors.toList());
      syntheticInterfaceMethods.add(
          new InterfaceMethodDefinition(
              sytheticMethodSignature,
              n.body().orElse(null),
              n.annotations(),
              n.modifiers(),
    ...
    if (!unitParameters.isEmpty() && !n.modifiers().contains(ConstructorModifier.PRIVATE)) {
      ...
      List<Expression> parameterBypass =
          n.signature().parameters().stream()
              .filter(p -> !unitParameters.contains(p.name()))
              .map(
                  p -> // unitParameters.contains( p.name() ) ?
      ...
      MethodCallExpression proxyConstructor =
          new MethodCallExpression(
              ...
              parameterBypass,
              Collections
                  .emptyList()); // TODO: fix this, we need to have the type parameters of the class
      syntheticConstructors.add(
