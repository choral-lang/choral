 ...
 *
 * <p>Returns a map, mapping each method in the CompilationUnit to a MiniZincInput.
 */
public class GenerateMiniZincInputs {
  ...
  /**
   * Generates a MiniZinc input for all the methods in the CompilationUnit. These inputs are stored
   * in the allInputs map.
   */
  private void buildMiniZincInput(CompilationUnit cu) {
    for (Pair<HigherCallable, Statement> methodPair : Utils.getMethods(cu)) {
      ...
      for (Entry<World, List<Pair<Expression, Statement>>> dependencySet :
      ...
      List<World> worlds =
          cu.classes().stream() // collect all roles in method
  ...
  private boolean isDuplicateDependency(
      Expression target, Map<Expression, Integer> allExpressions) {
  ...
  private class VisitStatement extends AbstractChoralVisitor<Void> {
    ...
    public Void visit(IfStatement n) {
      ...
      input.statements_blocks.add(currentBlock);
      World if_role =
          ((GroundDataType) n.condition().typeAnnotation().get()).worldArguments().get(0);
      ...
      // inserting the block into the list to make sure the index of the then-block matches
      // "then_block"
      input.blocks.add(
          new MiniZincInput.Block(
              if_start, 0, if_parent)); // the end of the block will be set later
      ...
      // inserting the block into the list to make sure the index of the else-block matches
      // "else_block"
      input.blocks.add(
          new MiniZincInput.Block(
              else_start, 0, if_parent)); // the end of the block will be set later
    ...
    public Void visit(ReturnStatement n) {
      ...
      input.statements_roles.add(
          ((GroundDataType) n.returnExpression().typeAnnotation().get()).worldArguments().get(0));
    ...
    /**
     * Insert a variable declaration into the MiniZinc input. Also visits the initializer if there
     * is one.
     */
    private void visitVariableDeclaration(VariableDeclaration vd, Statement n) {
      ...
      input.statements_blocks.add(currentBlock);
      input.statements_roles.add(
          new World(new Universe(), vd.type().worldArguments().get(0).name().identifier()));
  ...
  /**
   * Checks for nested dependencies and usage of variables. Adds to {@code dep_used_at} and sets
   * {@code dep_def_at} in the {@code MiniZincInput}.
   *
   * <p>Should called through {@code checkIfDependency()}
   */
  private class VisitExpression extends AbstractChoralVisitor<Void> {
    ...
    public Void visit(MethodCallExpression n) {
      for (Expression e : n.arguments()) checkIfDependency(e);
      return null;
    }
