...
public class GenerateMiniZincOutputs {
  ...
  private void callMiniZinc(CompilationUnit cu, Map<HigherCallable, MiniZincInput> inputs) {
    ...
    try {
      ...
      throw new CommunicationInferenceException(
          "An error occured while trying to create the MiniZinc file");
    }
    ...
    for (Pair<HigherCallable, Statement> methodPair : Utils.getMethods(cu)) {
      ...
      try {
        ...
        throw new CommunicationInferenceException(
            "An error occured while running the MiniZinc program on method " + method);
      }
    }
  }
  ...
  private void createMiniZincFile() throws IOException {
    ...
    for (String line : minizincReader.lines().toList()) minizincWriter.write(line + "\n");
    ...
    */
    ...
    for (int if_ = 0; if_ < input.num_ifs; if_++) {
      String[] ifSelections =
          allSelections
              .get(if_)
              .split("\\|"); // split takes a regex, so we need to escape the "|" character
