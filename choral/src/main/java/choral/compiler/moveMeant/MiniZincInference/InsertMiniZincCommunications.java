		...
		for( Class cls : old.classes() ) {
			...
			for( ConstructorDefinition constructor : cls.constructors() ) {
				...
				if( input == null )
					...
					newBody = new VisitStatement( input, output )
							.visitContinutation( constructor.blockStatements() );
				}
			...
			for( ClassMethodDefinition method : cls.methods() ) {
				...
				if( method.body().isPresent() ) {
					...
					if( input == null )
						...
						MiniZincOutput output = outputs.get( input );
						newBody = new VisitStatement( input, output )
								.visitContinutation( method.body().get() );
					}
	...
	private class VisitStatement extends AbstractChoralVisitor< Statement > {
		...
		public Statement visit( VariableDeclarationStatement n ) {
			...
			for( int i = 0; i < n.variables().size(); i++ ) {
				...
				vds.add( visitVariableDeclaration( statementIndex, n.variables().get( i ),
						used_at_n ) );
			}
		...
		public Statement visit( BlockStatement n ) {
			...
			// The before block statement is "{". No dependency can be used in this statement
			Statement comsBeforeEndOfBlock =
					createCommunications( beforeEndOfBlockIndex, n.position() );
		...
		private Statement createCommunications( Integer statementIndex, Position pos ) {
			...
			Statement dataCommunications = createDataCommunications(
					output.dataCommunications.get( statementIndex ), pos );
		...
		private Statement createSelections(
			...
			for( MiniZincSelectionMethod selectionMethod : selectionsToInsert ) {
				selectionExpressions.add(
						selectionMethod.createSelectionExpression( enum_, currentEnumConstant,
								pos ) );
			}
		...
		private Expression visitExpression(
				List< Dependency > dependencyList, Expression expression
		) {
			return new VisitExpression( output.dependencyVariables, dependencyList )
					.visit( expression );
		}
		 ...
		 */
		private Statement insertCommunicationsBefore(
				Statement communications, Statement statement
		) {
		 ...
		 * Returns statement if communications is null.
		 */
		private Statement insertCommunicationsAfter(
				Statement statement, Statement communications
		) {
			if( communications == null || communications instanceof NilStatement )
				return statement;
