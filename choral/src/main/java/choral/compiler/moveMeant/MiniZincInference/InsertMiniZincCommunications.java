...
public class InsertMiniZincCommunications {
  ...
  public InsertMiniZincCommunications(
      Map<HigherCallable, MiniZincInput> inputs, Map<MiniZincInput, MiniZincOutput> outputs) {
    this.inputs = inputs;
  ...
  private CompilationUnit createNewCompilationUnit(CompilationUnit old) {
    ...
    for (Class cls : old.classes()) {
      ...
      for (ConstructorDefinition constructor : cls.constructors()) {
        ...
        Statement newBody;
        if (input == null) newBody = constructor.blockStatements();
        else {
          MiniZincOutput output = outputs.get(input);
          newBody =
              new VisitStatement(input, output).visitContinutation(constructor.blockStatements());
        }
      ...
      for (ClassMethodDefinition method : cls.methods()) {
        ...
        if (method.body().isPresent()) {
          ...
          if (input == null) newBody = method.body().get();
          else {
            MiniZincOutput output = outputs.get(input);
    ...
    if (enum_ != null) newEnums.add(enum_); // insert enum used in selections
  ...
  private class VisitStatement extends AbstractChoralVisitor<Statement> {
    ...
    public Statement visit(TryCatchStatement n) {
      ...
      return new TryCatchStatement(
          visit(n.body()), n.catches(), visitContinutation(n.continuation()), n.position());
    }
     ...
     *
     * <p>Returns a statement containing the first communication. All other communications are
     * continuations of this statement. Returns null if there are no communications to be inserted.
     */
    private Statement createCommunications(Integer statementIndex, Position pos) {
      ...
      Statement dataCommunications =
          createDataCommunications(output.dataCommunications.get(statementIndex), pos);
      if (selections == null && dataCommunications == null) return null;
      if (selections == null) return dataCommunications;
      if (dataCommunications == null) return selections;
     ...
     *
     * <p>Returns a statement containing the first selection. All other selections are continuations
     * of this statement. Returns null if selectionsToInsert is null.
     */
    private Statement createSelections(
        List<MiniZincSelectionMethod> selectionsToInsert, Position pos) {
      if (selectionsToInsert == null) return null;
     ...
     *
     * <p>Returns a statement containing the first communication. All other communications are
     * continuations of this statement. Returns null if dependenciesToInsert is null.
     */
    private Statement createDataCommunications(
        List<Dependency> dependenciesToInsert, Position pos) {
      if (dependenciesToInsert == null) return null;
      ...
      if (variables.size() > 0) return chainVariables(variables, new NilStatement(pos));
      else return null;
    }

    /**
     * Creates a VariableDeclaration for the given dependency adds it to the given variables list.
     */
    private void createVariables(
        Dependency dependency, List<VariableDeclaration> variables, Position position) {
      ...
      Expression visitedDependencyExpression =
          new VisitExpression(output.dependencyVariables, nestedDependencies)
              .visit(dependency.originalExpression());
    ...
    /**
     * Create a {@code VariableDeclarationStatement} from each {@code VariableDeclaration} in
     * remainigVariables. Chains these {@code VariableDeclarationStatement}s together and set
     * continuation as the continuation of the last {@code VariableDeclarationStatement}.
     */
    ...
    /**
     * For the given {@code VariableDeclaration}, create the needed communications amd visits the
     * {@code VariableDeclaration}'s initializer
     */
    private Statement visitVariableDeclaration(
        Integer statementIndex, VariableDeclaration vd, List<Dependency> used_at_n) {
      Statement coms = createCommunications(statementIndex, vd.position());
      VariableDeclaration newVd;
      if (vd.initializer().isEmpty()) newVd = vd;
      else
        newVd =
            new VariableDeclaration(
                vd.name(),
      ...
      return insertCommunicationsBefore(
          coms,
          new VariableDeclarationStatement(
              List.of(newVd), new NilStatement(vd.position()), vd.position()));
    }

    /**
     * Places the communications {@code Statements} in front of the statement {@code Statement}.
     *
     * <p>Returns statement if communications is null.
     */
    private Statement insertCommunicationsBefore(Statement communications, Statement statement) {
      if (communications == null || communications instanceof NilStatement) return statement;
   ...
   *
   * <p>Iterates through {@code Expression}s and checks if they are equal to any of the nested
  ...
  private class VisitExpression extends AbstractChoralVisitor<Expression> {
    ...
    public VisitExpression(
        Map<Dependency, Name> dependencyVariables, List<Dependency> nestedDependencies) {
    ...
    /**
     * Checks if an {@code Expression} is a {@code Dependency}. If so, returns a {@code
     * FieldAccessExpression} accessing the variable in which the dependency is saved.
     *
     * <p>If the {@code Expression} is not a {@code Dependency}, return null.
     */
