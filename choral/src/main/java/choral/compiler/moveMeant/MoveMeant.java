...
public class MoveMeant {
  ...
  /**
   * Modifies the source CompilationUnits by inferring communications and selections, and inserting
   * them into method bodies.
   */
  public static List<CompilationUnit> infer(
      Collection<CompilationUnit> sources, Collection<CompilationUnit> headers, TyperOptions opts) {
    List<CompilationUnit> fullComCus = new ArrayList<>();

    if (INFERENCE_MODEL.equals("simple")) {
      Collection<CompilationUnit> dataComCus =
          sources.stream()
              .map(cu -> new VariableReplacement(new Selections()).inferComms(cu))
              .toList();
      ...
      throw new IllegalArgumentException(
          "Can't infer communications. Invalid inference model: " + INFERENCE_MODEL);
    }
