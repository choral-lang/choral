 ...
 *
 * <p>This expects that there are no dependencies on literals and that the resulting {@code
 * CompilationUnit} will be typed again, since most or all typeannotations will be lost
 */
public class VariableReplacement {
  ...
  /**
   * Iterates through all the methods of {@code cu} (including constructors), extracts their
   * dependencies, creates {@code Dependency} objects to store the dependencies and collects them in
   * {@code amendedStatements}.
   */
  private void buildAmendedStatements(CompilationUnit cu) {
    for (HigherCallable method : Utils.getJustMethods(cu)) {
      for (Entry<World, List<Pair<Expression, Statement>>> entryset :
          method.worldDependencies().entrySet()) {
        ...
        for (Pair<Expression, Statement> dependencyPair : entryset.getValue()) {
          ...
          // System.out.println( "Role " + receiver + " needs " + dependencyExpression + " from role
          // " + sender );
  ...
  /**
   * Returns the world from a dependency expression that needs to send data. If the dependency
   * expression has more than one sender world an error is thrown.
   *
   * <p>If the dependency expression is a method call, the type-annotation is set to be equal to the
   * method annotation.
   */
  private World getSender(Expression dependencyExpression) {
    ...
    if (dependencyExpression instanceof MethodCallExpression) {
      ...
      GroundDataType methodReturnType =
          (GroundDataType)
              ((MethodCallExpression) dependencyExpression).methodAnnotation().get().returnType();
   ...
   *
   * <p>We need to create a new {@code CompilationUnit} since everything in a {@code
   * CompilationUnit} (in particular {@code Statements} and {@code Expressions}) are final, and can
   ...
   *
   * <p>Iterates through {@code Statement}s and their continuations, and anytime a {@code Statement}
   * is in the {@code amendedStatements} map, the {@code Expression}s of that {@code Statement} are
   * visited by {@code VisitExpression}
   */
  private class VisitStatement extends AbstractChoralVisitor<Statement> {
    ...
    public Statement visit(ExpressionStatement n) {
      ...
      return new ExpressionStatement(
          newExpression, visitContinutation(n.continuation()), n.position());
    }
    ...
    public Statement visit(VariableDeclarationStatement n) {
      ...
      return new VariableDeclarationStatement(
          newVariables, visitContinutation(n.continuation()), n.position());
    }
    ...
    public Statement visit(BlockStatement n) {
      ...
      return new BlockStatement(
          n.enclosedStatement(), visitContinutation(n.continuation()), n.position());
    }
    ...
    public Statement visit(TryCatchStatement n) {
      ...
      if (dependencyList == null) {
        ...
        for (Pair<VariableDeclaration, Statement> pair : n.catches()) {
          newCatches.add(new Pair<>(pair.left(), visit(pair.right())));
        }
      } else {
        for (Pair<VariableDeclaration, Statement> pair : n.catches()) {
          newCatches.add(
              new Pair<>(
                  visitVariableDeclaration(dependencyList, pair.left()), visit(pair.right())));
        }
      }

      return new TryCatchStatement(
          visit(n.body()), newCatches, visitContinutation(n.continuation()), n.position());
    }
    ...
    private Statement visitContinutation(Statement continutation) {
      ...
      if (dependencyList == null) {
        ...
        if (variables.size() > 0) return chainVariables(variables, visit(continutation));
        else return visit(continutation);
      }
    }
    ...
    private boolean hasVariable(Dependency dependency) {
      ...
      for (Dependency solvedDependency : dependencyVariables.keySet()) {
        if (solvedDependency.originalExpression().toString().equals(dependencyString)) return true;
      }
      return false;
    }
    ...
    private void createVariables(
      ...
      variables.add(
          new VariableDeclaration(
              variableName, dependency.getType(), Collections.emptyList(), initializer, position));

      // System.out.println( "Dependency: Role " + dependency.recipient() + " needs " +
      // dependency.originalExpression() );
     ...
     *
     * <p>Puts the continuation as the continuation of the last VariableDeclaration in
     * remainingVariables.
     */
    private Statement chainVariables(
      ...
      VariableDeclarationStatement variableStaement =
          new VariableDeclarationStatement(
              List.of(variable), continuation, continuation.position());
      return chainVariables(remainingVariables, variableStaement);
    }

    /**
     * For some initial {@code Expression} and a list of {@code Dependency}s, visits the initial
     * {@code Expression} with the dependency list.
     *
    ...
    /**
     * If there is no initializer, return the given {@code VaraibleDeclaration} without change,
     * otherwise visit its initializer and return a new {@code VaraibleDeclaration}
     */
   ...
   *
   * <p>Iterates through {@code Expression}s and checks if they are equal to anything inside {@code
   * dependencyVariables}. If an {@code Expression} is equal to a dependency it is replaced with a
   * variable.
   */
  private class VisitExpression extends AbstractChoralVisitor<Expression> {
    ...
    public Expression visit(MethodCallExpression n) {
      ...
      return new MethodCallExpression(n.name(), newArgs, n.typeArguments(), n.position());
    }
    ...
    public Expression visit(AssignExpression n) {
      ...
      return new AssignExpression(newValue, n.target(), n.operator(), n.position());
    }
    ...
    public Expression visit(BinaryExpression n) {
      ...
      return new BinaryExpression(newLeft, newRight, n.operator(), n.position());
    }
    ...
    public Expression visit(EnclosedExpression n) {
      ...
      return new EnclosedExpression(newNestedExpression, n.position());
    }
    ...
    public Expression visit(NotExpression n) {
      ...
      return new NotExpression(newExpression, n.position());
    }
    ...
    /**
     * Checks if an {@code Expression} is a {@code Dependency}. If so, visits the {@code Expression}
     * and returns the visited expression wrapped in a communication, specified by the {@code
     * Dependency}. Otherwise returns null.
     */
  ...
  private class VisitDependency extends AbstractChoralVisitor<Void> {
    ...
    public Void visit(AssignExpression n) {
      // AsignExpressions should not be able to be dependencies, and thus should never be visited by
      // this.
      throw new UnsupportedOperationException(
  ...
  /**
   * A class for represent a dependency, consisting of the original dependency expression and a
   * communication (both channel and method) for satisfying this dependency.
   *
   * <p>Has method {@code createComExpression} for creating a communication expression out of the
   * stored communication and the provided {@code Expression} argument.
   */
  private class Dependency {
    ...
    /**
     * Creates the {@code Expression} containing the communiction of the dependency. Note that the
     * argument of the communication method ({@code visitedExpression}) must be visited before
     * creating the comExpression.
     *
     * <p>This expression needs
     *
     * <p>1. A name - The name of out communication method (com)
     *
     * <p>2. Arguments - Our {@code visitedDependency} expression. This is expected to be a visited
     * version of {@code originalExpression}. Note that this must be visited before calling {@code
     * createComExpression}. This is because we use java's {@code Object.equals()} to check if an
     * expression is a dependency. If {@code createComExpression} is called before visiting {@code
     * originalExpression} then dependencies inside {@code originalExpression} (nested dependencies)
     * will not be caught.
     *
     * <p>3. type argumetns - com methods always need the type of the data they are communicating.
     * This is stored as a {@code TypeExpression}. These TypeExpressions contain the unqualified
     * name of the type (e.g. not "java.lang.Object", only "Object") and composite types (types
     * containing other types (like {@code List})) also have a list of {@code TypeExpression}s
     * representing its inner types.
     *
     * @param visitedDependency - Must be visited before calling this method, otherwise nested
     *     dependencies will not be caught.
     */
    public Expression createComExpression(Expression visitedDependency) {
      ...
      if (originalExpression.typeAnnotation().get() instanceof GroundTypeParameter) {
        typeExpression =
            getTypeExpression((GroundTypeParameter) originalExpression.typeAnnotation().get());
      } else {
        typeExpression =
            getTypeExpression((GroundClassOrInterface) originalExpression.typeAnnotation().get());
      }
      ...
      MethodCallExpression scopedExpression =
          new MethodCallExpression(name, arguments, typeArguments, visitedDependency.position());

      // The comMethod is a method inside a channel, so we need to make the channel its scope
      FieldAccessExpression scope =
          new FieldAccessExpression(new Name(channelIdentifier), visitedDependency.position());
    ...
    private TypeExpression getTypeExpression(GroundClassOrInterface type) {
      return new TypeExpression(
          ...
          type.typeArguments().stream()
              .map(typeArg -> getTypeExpression(typeArg.applyTo(type.worldArguments())))
              .toList());
    }
    ...
    private TypeExpression getTypeExpression(GroundReferenceType type) {
      if (type
          instanceof
          GroundClass) { // I think this is only not true for primitive types, which cannot be
        // communicated
        GroundClass typeGC = (GroundClass) type;
        return new TypeExpression(
            new Name(typeGC.typeConstructor().identifier()),
            Collections.emptyList(),
            typeGC.typeArguments().stream()
                .map(typeArg -> getTypeExpression(typeArg.applyTo(type.worldArguments())))
                .toList());
      }
