...
public class Utils {
   ...
   *
   * <p>Returns a List of Pair of the methods typeannotation and the first statement in its body.
   *
   * <p>filters out all methods that has no method body.
   */
  public static List<Pair<HigherCallable, Statement>> getMethods(CompilationUnit cu) {
    return Stream.concat(
            cu.classes().stream()
                ...
                .filter(
                    method ->
                        method.body().isPresent() && !(method.body().get() instanceof NilStatement))
                .map(
                    method ->
                        new Pair<HigherCallable, Statement>(
                            method
                                .signature()
                                .typeAnnotation()
                                .get(), // we assume that methods are type-annotated
                            method.body().get())),
            cu.classes().stream()
                .flatMap(
                    cls ->
                        cls.constructors().stream()
                            .map(
                                method ->
                                    new Pair<HigherCallable, Statement>(
   ...
   * Retreives all methods from the {@code CompilationUnit} including constructors.
   *
   * <p>Returns a list of the methods' typeannotations.
   */
  ...
  /**
   * Searches through the methods of {@code channels} and returns the first viable com method based
   * on the input.
   *
   * <p>A viable method means:
   *
   * <ul>
   *   <li>{@code dependencyType} is a subtype of the channel's type
   *   <li>The method identifer is "com"
   *   <li>It takes a parameter at world {@code sender}
   *   <li>It returns something at world {@code recipient}
   * </ul>
   *
  ...
  public static Pair<Pair<String, GroundInterface>, HigherMethod> findComMethod(
    ...
    for (Pair<String, GroundInterface> channelPair : channels) {
      ...
      if (channel.typeArguments().stream()
          .anyMatch(typeArg -> dependencyType.typeConstructor().isSubtypeOf(typeArg))) {
                    ...
                    method ->
                        method.identifier().equals("com")
                            ...
                            ((GroundDataType) method.innerCallable().returnType())
                                ...
                                .get(0)
                                .equals(
                                    recipient)) // its returntype's world is equal to our dependency
                // recipient
                .findAny();
  ...
  /**
   * Searches through the methods of {@code channels} and returns the first viable selection method
   * based on the input.
   *
   * <p>A viable method means:
   *
   * <ul>
   *   <li>{@code dependencyType} is a subtype of the channel's type
   *   <li>The method identifer is "select"
   *   <li>It takes a parameter at world {@code sender}
   *   <li>It returns something at world {@code recipient}
   * </ul>
   *
  ...
  public static Pair<Pair<String, GroundInterface>, HigherMethod> findSelectionMethod(
      World recipient, World sender, List<Pair<String, GroundInterface>> channels) {
    for (Pair<String, GroundInterface> channelPair : channels) {
   ...
   *
   * <p>Two expressions are equal if ExpressionsMerger can successfully merge them.
   */
   ...
   *
   * <p>Given a list of statements {@code stms}, it returns the {@code stms[0]} with its
   * continuation set to {@code stms[1]}, and {@code stms[1]}'s continuation set to {@code stms[2]}
   * and so on.
   */
  ...
  public static Statement chainStatements(List<Statement> statements, Statement last) {
    if (statements.size() == 0) return last;
