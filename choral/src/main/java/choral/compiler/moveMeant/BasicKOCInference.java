...
public class BasicKOCInference {
	...
	private class VisitStatement extends AbstractChoralVisitor< Void > {
		...
		public Void visit( IfStatement n ) {
			var senders =
					( (GroundDataType) n.condition().typeAnnotation().get() ).worldArguments();
			if( senders.size() != 1 ) {
				throw new CommunicationInferenceException(
						"Found " + senders.size() + " roles, expected 1" );
			}
			World sender = senders.get( 0 );

			List< World > recipients =
					getParticipants( sender, List.of( n.ifBranch(), n.elseBranch() ) );
		 ...
		 */
		private List< World > getNeedsKOC(
				List< World > participants, List< Statement > statements
		) {
			...
			for( World participant : participants ) {
				...
				try {
					...
					List< Statement > projectedStatements = new ArrayList<>( statements ).stream()
							.map(
									statement -> StatementsProjector.visit(
											new WorldArgument(
													new Name( participant.identifier() ) ),
											statement ) )
							.toList();
		...
		private Pair< List< Expression >, List< Expression > > inferIfSelection(
			...
			for( SelectionMethod selectionMethod : selectionList ) {
				...
				ScopedExpression ifSelectionExpression = selectionMethod
						.createSelectionExpression( ifEnum, ifEnum.cases().get( 0 ) );
				ifSelections.add( ifSelectionExpression );
				// create selections for else branch
				ScopedExpression elseSelectionExpression = selectionMethod
						.createSelectionExpression( ifEnum, ifEnum.cases().get( 1 ) );
		...
		private List< SelectionMethod > findSelectionMethods(
			...
			if( !recipients.isEmpty() ) {
				...
				for( World recipient : recipients ) {
					exceptions.add( new CommunicationInferenceException(
							"No viable selection method was found for " + recipient
									+ " with sender " + initialSender ) );
				}
		...
		private SelectionMethod findSelectionMethod( World sender, World recipient ) {
			for( Pair< String, GroundInterface > channelPair : channels ) {
				...
				if( selectMethodOptional.isPresent() ) {
					return new SelectionMethod( channelPair.left(), channelPair.right(),
							selectMethodOptional.get(), sender );
				}
			}
		...
		private Enum getEnum( int numCases ) {
			...
			if( enumerator == null || enumerator.cases().size() < numCases ) {
				...
				for( int i = 0; i < numCases; i++ ) {
					cases.add( new EnumConstant( new Name( "CASE" + i ), Collections.emptyList(),
							null ) );
				}
	...
	private class SelectionMethod {
		...
		public ScopedExpression createSelectionExpression(
				Enum enumerator, EnumConstant enumCons
		) {
			...
			MethodCallExpression scopedExpression =
					new MethodCallExpression( name, arguments, typeArguments, position ); // TODO proper position
	...
	private class GetExpressionParticipants extends AbstractChoralVisitor< Void > {
		...
		public Void visit( MethodCallExpression n ) {
			if( !n.methodAnnotation().get().returnType().isVoid() ) {
				GroundDataType returnType =
						(GroundDataType) n.methodAnnotation().get().returnType();
