...
/**
 * Knowledge of choice inference. Iterates through a program and at every IfStatement and
 * SwitchStatement sends a selection to every role but itself, that takes part in at least one
 * branch in that Statement. Returns a Selections object with all the selections to insert.
 */
public class BasicKOCInference {
   ...
   *
   * <p>Also creates an enumerator of size equal to number of cases in the largest SwitchStatement,
   * or size two if only IfStatements are found, unless an enumerator of sufficient size has already
   * been created.
   *
   * <p>If no selections are needed, the selections map remains unchanged and no enumerator will be
   * created.
   */
  private class VisitStatement extends AbstractChoralVisitor<Void> {
    ...
    /**
     * takes a list of worlds and statements and returns the worlds that need knowledge of choice
     * based on the projection of the statements on the participants
     */
    private List<World> getNeedsKOC(List<World> participants, List<Statement> statements) {
      ...
      for (World participant : participants) {
        ...
        try {
          ...
          List<Statement> projectedStatements =
              new ArrayList<>(statements)
                  .stream()
                      .map(
                          statement ->
                              StatementsProjector.visit(
                                  new WorldArgument(new Name(participant.identifier())), statement))
                      .toList();
          ...
          // }
            ...
            Exception
                e) { // since not all implementations of merge() throws MergeException, we match on
          // all exceptions
    ...
    private Pair<List<Expression>, List<Expression>> inferIfSelection(
        World sender, List<World> recipients) {
      ...
      for (SelectionMethod selectionMethod : selectionList) {
        ...
        ScopedExpression ifSelectionExpression =
            selectionMethod.createSelectionExpression(ifEnum, ifEnum.cases().get(0));
        ifSelections.add(ifSelectionExpression);
        // create selections for else branch
        ScopedExpression elseSelectionExpression =
            selectionMethod.createSelectionExpression(ifEnum, ifEnum.cases().get(1));
     ...
     *
     * <p>Throws an error if at least one recipient cannot be reached.
     */
    private List<SelectionMethod> findSelectionMethods(
        World initialSender, List<World> recipientsList) {
        ...
        for (World recipient : recipients) {
          exceptions.add(
              new CommunicationInferenceException(
                  "No viable selection method was found for "
                      + recipient
                      + " with sender "
                      + initialSender));
        }
      ...
      for (Pair<String, GroundInterface> channelPair : channels) {
        ...
        Optional<? extends HigherMethod> selectMethodOptional =
            channelPair
                ...
                .filter(
                    method ->
                        method.identifier().equals("select")
                            && // it is a selection method (only checked through name)
                            method
                                .innerCallable()
                                .signature()
                                .parameters()
                            ...
                            method.innerCallable().returnType() instanceof GroundDataType
                            && // probably redundant check, returntype should not be able to be void
                            ((GroundDataType) method.innerCallable().returnType())
                                .worldArguments()
                                .get(0)
                                .equals(
                                    recipient)) // its returntype's world is equal to our recipient
                .findAny();

        if (selectMethodOptional.isPresent()) {
          return new SelectionMethod(
              channelPair.left(), channelPair.right(), selectMethodOptional.get(), sender);
        }
      }
  ...
  private class SelectionMethod {
    ...
    public ScopedExpression createSelectionExpression(Enum enumerator, EnumConstant enumCons) {
      ...
      TypeExpression typeExpression =
          new TypeExpression(
              enumerator.name(),
              Collections.emptyList(), // This needs to be "higher kinded" and can thus not have a
              // worldargument
              Collections.emptyList(),
              position); // TODO proper position

      TypeExpression argScope =
          new TypeExpression(
              enumerator.name(),
              List.of(
                  new WorldArgument(new Name(sender.identifier()))), // this needs a worldargument
              Collections.emptyList(),
              position); // TODO proper position

      ScopedExpression argument =
          new ScopedExpression( // looks something like Enum@Sender.CHOICE
              new StaticAccessExpression( // Enum@Sender
                  argScope, position), // TODO proper position
              new FieldAccessExpression( // CHOICE
                  enumCons.name(), position), // TODO proper position
              position); // TODO proper position
      ...
      MethodCallExpression scopedExpression =
          new MethodCallExpression(
              name, arguments, typeArguments, position); // TODO proper position

      // The selection method is a method inside its channel, so we need to make the channel its
      // scope
      FieldAccessExpression scope =
          new FieldAccessExpression(new Name(channelIdentifier), position); // TODO proper position
   ...
   *
   * <p>Use it be calling getParticipants() on some statement.
   */
  private class GetStatementParticipants extends AbstractChoralVisitor<Void> {
    ...
    @Override
    public Void visit(ExpressionStatement n) {
      Set<World> expressionParticipants =
          new GetExpressionParticipants().GetParticipants(n.expression());
    ...
    @Override
    public Void visit(IfStatement n) {
      Set<World> conditionParticipants =
          new GetExpressionParticipants().GetParticipants(n.condition());
    ...
    @Override
    public Void visit(ReturnStatement n) {
      Set<World> returnParticipants =
          new GetExpressionParticipants().GetParticipants(n.returnExpression());
    ...
    private Void visitVariableDeclaration(VariableDeclaration vd) {
      ...
      if (!vd.initializer().isEmpty()) {
        Set<World> initializerParticipants =
            new GetExpressionParticipants().GetParticipants(vd.initializer().get());
   ...
   *
   * <p>Use it be calling getParticipants() on some Expression.
   */
  private class GetExpressionParticipants extends AbstractChoralVisitor<Void> {
    ...
    public Void visit(FieldAccessExpression n) {
      GroundDataType nType =
          (GroundDataType) n.typeAnnotation().get(); // assuming that a field cannot be void
    ...
    public Void visit(LiteralExpression.BooleanLiteralExpression n) {
      participants.add(
          n.world().typeAnnotation().get()); // TODO check worldargumetns for everything above this
      return null;
    }
