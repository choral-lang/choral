...
public class DependencyVisitor implements ChoralVisitorInterface<Void> {
  ...
  public DependencyVisitor(List<CompilationUnit> CUs, WorldArgument w) {
    ...
    CUs.forEach(
        cu -> {
          cu.interfaces()
              .forEach(
                  i -> availableTemplates.put(i.name().identifier(), new AvailableTemplate(i, cu)));
          cu.classes()
              .forEach(
                  c -> availableTemplates.put(c.name().identifier(), new AvailableTemplate(c, cu)));
          cu.enums()
              .forEach(
                  e -> availableTemplates.put(e.name().identifier(), new AvailableTemplate(e, cu)));
        });
  }
  ...
  private Set<Pair<String, Integer>> collectProjectableTemplates(String template) {
    if (availableTemplates.containsKey(template)) {
      ...
      return projectableTemplates.stream()
          .filter(p -> availableTemplates.containsKey(p.left()))
          .collect(Collectors.toSet());
    } else {
  ...
  public Void visit(Class n) {
    ...
    if (toWorldArguments(n.worldParameters()).contains(w)) {
      ...
      visitedTemplates.add(
          new Pair<>(n.name().identifier(), toWorldArguments(n.worldParameters()).indexOf(w)));
    }
    return null;
  }
  ...
  public Void visit(EnumCaseInstantiationExpression n) {
    if (n.world().equals(w)) {
      projectableTemplates.add(
          new Pair<>(
              n.name().identifier(), 0 // we just have one world
              ));
    }
    return null;
  }
  ...
  public Void visit(TypeExpression n) {
    ...
    if (n.worldArguments().size() == 0
        && !n.name().identifier().equals("void")) { // this is of kind @ => ...
