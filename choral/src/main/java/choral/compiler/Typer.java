...
public class Typer {
  ...
  public static Collection<CompilationUnit> annotate(
    ...
    headerUnits.forEach(
        cu -> taskQueue.enqueue(Phase.TYPE_SYMBOL_DECLARATIONS, () -> headerVisitor.visit(cu)));
    Visitor sourceVisitor = new SourceVisitor(taskQueue, universe, opts);
    sourceUnits.forEach(
        cu -> taskQueue.enqueue(Phase.TYPE_SYMBOL_DECLARATIONS, () -> sourceVisitor.visit(cu)));
    taskQueue.process();
    return sourceUnits;
  }
  ...
  private abstract static class Visitor {
    ...
    private void checkExtendsForCycles(
      ...
      if (!supertype.isInheritanceFinalised()) {
        TaskQueue.Task hs = taskQueue.hierarchyConstructionTasks.get(supertype.typeConstructor());
          ...
          new TaskQueue.Task(
              ...
              () -> {
                ...
                for (choral.ast.type.TypeExpression x : n.implementsInterfaces()) {
                  try {
                    GroundInterface s =
                        visitGroundInterfaceExpression(
                            classOrInterfaceStaticScope.getInstanceScope(), x, true);
                ...
                visitTypeParametersBound(
                    classOrInterfaceStaticScope.getInstanceScope(), n.typeParameters(), true);
      ...
      taskQueue.enqueue(
          ...
          () -> {
            ...
            for (ClassMethodDefinition nm : n.methods()) {
              ...
              if (ms.contains(Modifier.ABSTRACT) && !t.isAbstract()) {
                throw new AstPositionedException(
                    nm.position(),
                    new StaticVerificationException("abstract method in non-abstract class"));
              }
              List<HigherTypeParameter> typeParams =
                  visitTypeParameters(nm.signature().typeParameters());
              Member.HigherMethod tm =
                  new Member.HigherMethod(
                      t.innerType(), nm.signature().name().identifier(), ms, typeParams);
              nm.setTypeAnnotation(tm);
              tm.setSourceCode(nm);
              ...
              visitTypeParametersBound(callableScope, nm.signature().typeParameters(), true);
              ...
              if (r.name().identifier().equals("void")) {
                ...
                tm.innerCallable()
                    .setReturnType(
                        visitGroundDataTypeExpression(
                            callableScope, nm.signature().returnType(), false));
              }
              ...
              taskQueue.enqueue(
                  Phase.MEMBER_DEFINITIONS,
                  () -> {
                    try {
                      visitMethodBody(callableScope.getScope(), tm, nm.body().orElse(null));
                    } catch (StaticVerificationException e) {
                      throw new AstPositionedException(nm.position(), e);
                    }
                  });
            ...
            for (ConstructorDefinition nm : n.constructors()) {
              ...
              List<HigherTypeParameter> typeParams =
                  visitTypeParameters(nm.signature().typeParameters());
              Member.HigherConstructor tm =
                  new Member.HigherConstructor(t.innerType(), ms, typeParams);
              nm.setTypeAnnotation(tm);
              tm.setSourceCode(nm);
              CallableScope callableScope =
                  classOrInterfaceStaticScope.getInstanceScope().getScope(tm);
              visitTypeParametersBound(callableScope, nm.signature().typeParameters(), false);
              for (FormalMethodParameter x : nm.signature().parameters()) {
                tm.innerCallable()
                    .signature()
                    .addParameter(
            ...
            taskQueue.enqueue(
                Phase.MEMBER_GLOBAL_CHECKS,
                () ->
                    checkConstructorsDependencies(
    ...
    private void visitEnum(
      ...
      if (n.worldParameters().size() != 1) {
        throw new AstPositionedException(
            (n.worldParameters().isEmpty()) ? n.position() : n.worldParameters().get(1).position(),
            new StaticVerificationException("enums must have exactly one role"));
      }
      HigherEnum t =
          new HigherEnum(pkg, modifiers, name, visitWorldParameter(n.worldParameters().get(0)), n);
      n.setTypeAnnotation(t); // annotate AST
      TaskQueue.Task ht =
          new TaskQueue.Task(
              Phase.HIERARCHY,
      ...
      taskQueue.enqueue(
          ...
          () -> {
            for (InterfaceMethodDefinition nm : n.methods()) {
              ...
              nm.setTypeAnnotation(tm);
              tm.setSourceCode(nm);
              CallableScope methodScope =
                  (ms.contains(Modifier.STATIC))
                      ? classOrInterfaceStaticScope.getScope(tm)
                      : classOrInterfaceStaticScope.getInstanceScope().getScope(tm);
              visitTypeParametersBound(methodScope, nm.signature().typeParameters(), true);
              for (FormalMethodParameter x : nm.signature().parameters()) {
                tm.innerCallable()
                    .signature()
                    .addParameter(
              ...
              if (r.name().identifier().equals("void")) {
                ...
                tm.innerCallable()
                    .setReturnType(
                        visitGroundDataTypeExpression(
                            methodScope, nm.signature().returnType(), false));
              }
              checkIfSelectionMethod(tm, nm.annotations());
              checkIfTypeSelectionMethod(tm, nm.annotations());
              tm.innerCallable().finalise();
              ...
              taskQueue.enqueue(
                  ...
                  () -> {
                    try {
                      visitMethodBody(methodScope.getScope(), tm, nm.body().orElse(null));
                    } catch (StaticVerificationException e) {
                      throw new AstPositionedException(nm.position(), e);
                    }
                  });
    ...
    private void checkIfSelectionMethod(Member.HigherMethod tm, List<Annotation> annotations) {
      for (Annotation x : annotations) {
        if (x.getName().identifier().equals(SELECTION_METHOD_ANNOTATION)) {
          ...
          GroundDataType mp = tm.innerCallable().signature().parameters().get(0).type();
          ...
          if (tr.worldArguments().size() != 1
              || !tr.isEquivalentTo(tp.applyTo(tr.worldArguments()))) {
            List<World> ws = World.freshWorlds(universe(), 1, "X");
            throw new AstPositionedException(
                x.position(),
                new StaticVerificationException(
    ...
    private void checkIfTypeSelectionMethod(Member.HigherMethod tm, List<Annotation> annotations) {
      for (Annotation x : annotations) {
    ...
    private List<HigherTypeParameter> visitTypeParameters(
      ...
      for (FormalTypeParameter m : typeParameters) {
        HigherTypeParameter t =
            new HigherTypeParameter(
                universe, m.name().identifier(), visitWorldParameters(m.worldParameters()), m);
        result.add(t);
    ...
    private void visitTypeParametersBound(
      ...
      for (FormalTypeParameter n : typeParameters) {
        ...
        for (TypeExpression m : n.upperBound()) {
          p.innerType()
              .addUpperBound(visitGroundReferenceTypeExpression(scope, m, delayBoundChecks));
        }
    ...
    private Type visitTypeExpression(Scope scope, TypeExpression n, boolean delayBoundChecks) {
      HigherDataType type = scope.assertLookupDataType(n.name().identifier());
      List<World> worldArgs =
          n.worldArguments().stream()
              .map(
                  x ->
                      scope
                          .lookupWorldParameter(x.name().identifier())
                          .orElseThrow(
                              () ->
      ...
      List<HigherReferenceType> typeArgs =
    ...
    protected HigherReferenceType visitHigherReferenceTypeExpression(
      ...
      try {
        return assertHigherReferenceType(visitTypeExpression(scope, n, delayBoundChecks));
      } catch (StaticVerificationException e) {
        throw new AstPositionedException(n.position(), e);
      }
    }
    ...
    protected GroundDataType visitGroundDataTypeExpression(
        Scope scope, TypeExpression n, boolean delayBoundChecks) {
      try {
        return assertGroundDataType(visitTypeExpression(scope, n, delayBoundChecks));
      } catch (StaticVerificationException e) {
        throw new AstPositionedException(n.position(), e);
      }
    }
    ...
    protected GroundReferenceType visitGroundReferenceTypeExpression(
        Scope scope, TypeExpression n, boolean delayBoundChecks) {
      try {
        return assertGroundReferenceType(visitTypeExpression(scope, n, delayBoundChecks));
      } catch (StaticVerificationException e) {
        throw new AstPositionedException(n.position(), e);
      }
    }
    ...
    protected GroundInterface visitGroundInterfaceExpression(
        Scope scope, TypeExpression n, boolean delayBoundChecks) {
      try {
        return assertGroundInterface(visitTypeExpression(scope, n, delayBoundChecks));
      } catch (StaticVerificationException e) {
        throw new AstPositionedException(n.position(), e);
      }
    }
    ...
    private static HigherReferenceType assertHigherReferenceType(Type t) {
      if (t instanceof HigherReferenceType) {
        ...
        throw new StaticVerificationException(
            formattedAssertTypeMessage(t, "higher-kinded reference type expected, '%1$s' is %3$s"));
      }
    }
    ...
    private static GroundReferenceType assertGroundReferenceType(Type t) {
      if (t instanceof GroundReferenceType) {
        ...
        throw new StaticVerificationException(
            formattedAssertTypeMessage(t, "reference type expected, '%1$s' is %3$s"));
      }
    }
  ...
  private static class HeaderVisitor extends Visitor {
    ...
    @Override
    protected void checkPrimaryTemplate(TemplateDeclaration n, CompilationUnit cu, String family) {
  ...
  private static class SourceVisitor extends Visitor {
      ...
      while (!q.isEmpty()) {
        ...
        while (c != null) {
          if (v.contains(c)) {
            throw new AstPositionedException(
                positions.get(c),
                new StaticVerificationException("recursive constructor invocation"));
          }
          v.add(c);
          q.remove(c);
    ...
    protected void visitMethodBody(
      ...
      if (callable.isAbstract()) {
        if (body != null) {
          throw new AstPositionedException(
              body.position(),
              new StaticVerificationException("abstract methods cannot have bodies"));
        }
        // nothing to check :)
      } else {
        if (body == null) {
          throw new StaticVerificationException("non-abstract methods must have bodies");
        } else {
          callable.addChannel(bodyScope.getChannels());
          boolean returnChecked =
              check(body, callable.innerCallable().returnType(), bodyScope, callable);
          if (!callable.innerCallable().returnType().isVoid() && !returnChecked) {
            throw new AstPositionedException(
                body.position(), new StaticVerificationException("missing return statement"));
          }
        }
      }
    }
      ...
      if (d.explicitConstructorInvocation().isEmpty()) {
        ...
        List<? extends GroundDataType> args =
            n.arguments().stream()
                ...
                .collect(Collectors.toList());
        List<? extends Member.GroundCallable> ms =
            findMostSpecificCallable(
                typeArgs,
                args,
    ...
    private List<? extends Member.GroundCallable> findMostSpecificCallable(
      ...
      List<? extends Member.HigherCallable> cs =
          callables
              .filter(x -> x.typeParameters().size() == typeArgs.size() && x.arity() == args.size())
              .collect(Collectors.toList());
      ...
      do {
        ...
        for (Member.HigherCallable x : cs) {
          ...
          for (int i = 0; i < cparams.size(); i++) {
            ...
            if (phase == 1) {
              incompatible = opts.relaxed() ? !a.isSubtypeOf_relaxed(p) : !a.isSubtypeOf(p);
            } else {
              if (p.isPrimitive()) {
              ...
              // ) );
              incompatible = opts.relaxed() ? !a.isAssignableTo_relaxed(p) : !a.isAssignableTo(p);
            }
            if (incompatible) {
              break;
            }
    ...
    /**
     * In the "relaxed" typer mode, there might be multiple methods that have exactly the same
     * datatypes as parameters and differ only in worlds: for example, {@code myMethod(int@A,
     * bool@B)} and {@code myMethod(int@B, bool@A)}. If that happens, we use the DWIM ("Do What I
     * Mean") resolution strategy explained below.
     *
     * <p>If there's a method whose parameter worlds match the argument worlds exactly, e.g. {@code
     * myMethod(int@A, bool@B)} with invocation {@code myMethod(1@A, true@B}, we assume that's the
     * method they wanted to invoke---even though they could conceivably have wanted us to infer
     * this: {@code myMethod(chAB.com(1@A), chBA.com(true@B)}.
     *
     * <p>We also go a step further. Maybe there's a method that matches all argument worlds
     * exactly, except for the i-th argument. So the i-th argument will have to be sent to some
     * other world---but which one? Well, if all method overloads have the i-th parameter at the
     * same world, then our choice is deterministic. So if we invoke {@code myMethod(1@A, true@A)}
     * and our overloads are {@code myMethod(int@A, bool@B)} and {@code myMethod(int@B, bool@B)},
     * then we'll assume they wanted us to infer {@code myMethod(1@A, chAB.com(true@A))}---even
     * though they could conceivably have wanted us to infer {@code myMethod(chAB.com(1@A),
     * chAB.com(true@A))}.
     */
    private static List<Member.GroundCallable> filterMethodsUsingWorlds(
        List<Member.GroundCallable> methods, List<? extends GroundDataType> args) {
      ...
      for (int i = 0; i < args.size(); i++) {
        ...
        if (eligibleMethods.isEmpty()) {
          ...
          boolean allAtSameWorld =
              methods.stream()
                  .allMatch(method -> getParamWorlds(method, index).equals(firstWorlds));
          if (!allAtSameWorld) {
    ...
    /**
     * Returns the list of worlds associated with the i-th parameter of the given method. For
     * example, the 1-th world of {@code myMethod(int@A, bool@B)} is B.
     */
    ...
    GroundDataType assertNotVoid(GroundDataTypeOrVoid t, Position position) {
      if (t.isVoid()) {
        throw new AstPositionedException(
            position, new StaticVerificationException("data type expected, found 'void'"));
      } else {
        return (GroundDataType) t;
      }
    }
    ...
    private final class Check extends AbstractChoralVisitor<Boolean> {
          ...
          if (entry.getKey() instanceof SwitchArgument.SwitchArgumentLabel label) {
            if (guard.isEnum()) {
              ...
              if (ge.field(id).isEmpty()) {
                throw new AstPositionedException(
                    label.argument().position(), new UnresolvedSymbolException(id));
              } else {
                if (!casesFound.add(id)) {
                  throw new AstPositionedException(
                      label.argument().position(),
                      new StaticVerificationException("duplicate case '" + id + "'"));
                }
              }
            } else {
              throw new AstPositionedException(
            ...
            if (!argument.isAssignableTo(guard)) {
              throw new AstPositionedException(
                  l.position(),
                  new StaticVerificationException(
                      "required type '" + guard + "', found '" + guard + "'"));
            }
            if (!casesFound.add(s)) {
              throw new AstPositionedException(
                  l.argument().position(),
      ...
      public Boolean visit(TryCatchStatement n) {
        ...
        for (Pair<VariableDeclaration, Statement> c : n.catches()) {
          ...
          GroundDataType te = visitGroundDataTypeExpression(scope, c.left().type(), false);
          GroundClassOrInterface expectedType =
              universe().specialType(SpecialTypeTag.EXCEPTION).applyTo(te.worldArguments());
          boolean isSubtype =
              opts.relaxed() ? te.isSubtypeOf_relaxed(expectedType) : te.isSubtypeOf(expectedType);
          // exceptions only allowed one role
          if (te.worldArguments().size() > 1 || !isSubtype) {
            throw new AstPositionedException(
                c.left().type().position(),
                new StaticVerificationException(
                    "required an instance of type '"
      ...
      public Boolean visit(ReturnStatement n) {
        if (n.returnExpression() == null) {
          if (expected == universe().voidType()) {
            ...
            throw new AstPositionedException(
                n.position(), new StaticVerificationException("missing return value"));
          }
        } else {
      ...
      GroundDataTypeOrVoid synth(Expression n, Statement statement) {
        return new Synth(scope, false, Collections.emptyList(), enclosingMethod, statement, opts)
            .visit(n);
      }

      GroundDataTypeOrVoid synth(
          Expression n, Statement statement, List<? extends World> homeWorlds) {
        return new Synth(scope, false, homeWorlds, enclosingMethod, statement, opts).visit(n);
      }
    }
    ...
    private final class Synth extends AbstractChoralVisitor<GroundDataTypeOrVoid> {
      ...
      /**
       * The set of worlds where we think the expression will be evaluated. For example, in the
       * statement {@code int@B z = x + y;}, we set B as the home world. We use this information for
       * communication inference: if x was located at another world A, we'd infer that B needs a
       * copy of x from A.
       */
      ...
      private GroundPrimitiveDataType assertUnbox(GroundDataTypeOrVoid type, Position position) {
      ...
      private GroundDataType visitBinaryOp(
        ...
        if (tvl.isVoid() || tvr.isVoid()) {
          throw new AstPositionedException(
              position,
              new StaticVerificationException(
                  "cannot apply '" + operator + "' to '" + tvl + "' and '" + tvr + "'"));
        }
        ...
        switch (operator) {
          case PLUS:
            {
              if (leftType.specialTypeTag() == SpecialTypeTag.STRING
                ...
                return universe().specialType(SpecialTypeTag.STRING).applyTo(worlds);
              }
            }
          case MINUS:
          case MULTIPLY:
          ...
          case REMAINDER:
            ...
            if (pl.primitiveTypeTag().isNumeric() && pr.primitiveTypeTag().isNumeric()) {
              GroundPrimitiveDataType p =
                  (pl.primitiveTypeTag().compareTo(pr.primitiveTypeTag()) > 0) ? pl : pr;
              if (p.primitiveTypeTag().compareTo(PrimitiveTypeTag.INT) < 0) {
                p = universe().primitiveDataType(PrimitiveTypeTag.INT).applyTo(worlds);
              }
              return p;
            }
            break;
          ...
          case GREATER_EQUALS:
            ...
            pr = assertUnbox(rightType, position);
            if (pl.primitiveTypeTag().isNumeric() && pr.primitiveTypeTag().isNumeric()) {
              return universe().primitiveDataType(PrimitiveTypeTag.BOOLEAN).applyTo(worlds);
            }
            break;
          ...
          case NOT_EQUALS:
            boolean trSubtype =
                opts.relaxed()
                    ? rightType.isSubtypeOf_relaxed(leftType)
                    : rightType.isSubtypeOf(leftType);
            boolean tlSubtype =
                opts.relaxed()
                    ? leftType.isSubtypeOf_relaxed(rightType)
      ...
      public GroundDataType visit(AssignExpression n) {
        ...
        if (tvl.isVoid()) {
          throw new AstPositionedException(
              n.position(), new StaticVerificationException("expected assignable variable"));
        }
        ...
        if (tvr.isVoid()) {
          throw new AstPositionedException(
              n.position(),
              new StaticVerificationException("required type '" + tl + "', found '" + tvr + "'"));
        }
        ...
        boolean assignable = opts.relaxed() ? tr.isAssignableTo_relaxed(tl) : tr.isAssignableTo(tl);
        if (!assignable) {
          throw new AstPositionedException(
              n.position(),
              new StaticVerificationException("required type '" + tl + "', found '" + tr + "'"));
        }
      ...
      public GroundDataType visit(FieldAccessExpression n) {
        ...
        leftStatic = false;
        if (result.isEmpty()) {
          throw new AstPositionedException(n.position(), new UnresolvedSymbolException(identifier));
        } else {
      ...
      public GroundDataType visit(StaticAccessExpression n) {
        ...
        if (!m.typeArguments().isEmpty()) {
          throw new AstPositionedException(
              m.typeArguments().get(0).position(),
              new StaticVerificationException("unexpected type argument in static member access"));
        }
      ...
      public GroundDataType visit(ClassInstantiationExpression n) {
        ...
        if (t.typeConstructor().isAbstract()) {
          throw new AstPositionedException(
              n.position(),
              new StaticVerificationException("'" + t + "' is abstract, cannot be instantiated"));
        }
            ...
            throw new AstPositionedException(
                ...
                new StaticVerificationException(
                    "ambiguous method invocation, "
                        + ms.stream()
                            .map(Member.GroundCallable::toString)
                            .collect(
                                Collectors.collectingAndThen(
                                    Collectors.toList(), Formatting.joiningOxfordComma()))));
          }
       ...
       * ClassInstantiationExpression. Type arguments are stored in the left value, and regular
       * arguments in the right value.
       */
      public Pair<List<? extends HigherReferenceType>, List<? extends GroundDataType>>
        ...
        List<? extends HigherReferenceType> typeArgs =
            expression.typeArguments().stream()
                .map(x -> visitHigherReferenceTypeExpression(scope, x, false))
                .collect(Collectors.toList());
        List<? extends GroundDataType> args =
            expression.arguments().stream()
                .map(x -> assertNotVoid(synth(x, Collections.emptyList()), x.position()))
                .collect(Collectors.toList());
      ...
      public GroundDataType visit(NotExpression n) {
        ...
        if (p.primitiveTypeTag() == PrimitiveTypeTag.BOOLEAN) {
          ...
          throw new AstPositionedException(
              n.position(), new StaticVerificationException("cannot apply '!' to '" + t + "'"));
        }
      }
      ...
      public GroundDataType visit(LiteralExpression.BooleanLiteralExpression n) {
        ...
        return annotate(
            n,
            universe().primitiveDataType(PrimitiveTypeTag.BOOLEAN).applyTo(visitWorld(n.world())));
      }
      ...
      public GroundDataType visit(LiteralExpression.IntegerLiteralExpression n) {
        ...
        return annotate(
            n, universe().primitiveDataType(PrimitiveTypeTag.INT).applyTo(visitWorld(n.world())));
      }
      ...
      public GroundDataType visit(LiteralExpression.DoubleLiteralExpression n) {
        ...
        return annotate(
            n,
            universe().primitiveDataType(PrimitiveTypeTag.DOUBLE).applyTo(visitWorld(n.world())));
      }
      ...
      public GroundDataType visit(LiteralExpression.StringLiteralExpression n) {
        ...
        return annotate(
            n,
            universe().specialType(SpecialTypeTag.STRING).applyTo(List.of(visitWorld(n.world()))));
      }
      ...
      /**
       * In the "relaxed" typing mode, check if an expression's location matches its expected
       * location. If it doesn't, record the expression as a dependency.
       *
       * @param type The expression's type
       * @param toWorlds The place where the expression's result should be located
       */
      private void recordDependencies(
          Expression expression, GroundDataTypeOrVoid type, List<? extends World> toWorlds) {
      ...
      /**
       * In the "relaxed" typing mode, check the locations of the arguments match the locations
       * expected by the method. Any argument that doesn't match is recorded as a dependency for
       * communication inference.
       *
       * @param method The method being invoked
       * @param args The arguments passed to the method; we assume the arguments already have type
       *     annotations
       */
      private void recordDependencies(Member.GroundCallable method, List<Expression> args) {
        ...
        if (!homeWorlds.isEmpty() && !homeWorlds.contains(visitWorld(n.world()))) {
          throw new AstPositionedException(
              ...
              new StaticVerificationException(
                  "Literal '"
                      + n
                      + "', can't be used in an expression at role '"
                      + homeWorlds
                      + "'"));
        }
      }
