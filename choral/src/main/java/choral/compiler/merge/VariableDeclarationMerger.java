...
class VariableDeclarationMerger extends AbstractMerger<VariableDeclaration> {
  ...
  static VariableDeclaration mergeVariableDeclarations(
    ...
    MergeException._assert(
        ...
        errorPrefix + "different variable names: " + n1.name() + " and " + n2.name(),
        n1,
        n2);
    MergeException._assert(
        n1.type().equals(n2.type()),
        errorPrefix + "different types:  " + n1.type() + " and " + n2.type(),
        n1,
        n2);
    MergeException._assert(
        n1.annotations().equals(n2.annotations()),
        errorPrefix
            + "different annotation lists: "
            + n1.annotations()
            + " and "
            + n2.annotations(),
        n1,
    ...
    MergeException._assert(
        ...
        errorPrefix + "different initializers: " + i1.orElse(null) + " and " + i2.orElse(null),
        n1,
        n2);
    ...
    return new VariableDeclaration(
        ...
        i1.isPresent() && i2.isPresent()
            ? (AssignExpression) ExpressionsMerger.mergeExpressions(i1.get(), i2.get())
            : null);
  }
}
