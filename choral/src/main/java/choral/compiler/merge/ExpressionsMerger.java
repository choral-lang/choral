...
public class ExpressionsMerger extends AbstractMerger<Expression> {
  ...
  private final PrettyPrinterVisitor ppv = new PrettyPrinterVisitor();
  ...
  public Expression merge(AssignExpression n1, AssignExpression n2) {
    ...
    return new AssignExpression(
        merge(n1.value(), n2.value()), merge(n1.target(), n2.target()), n1.operator());
  }
  ...
  public Expression merge(BinaryExpression n1, BinaryExpression n2) {
    ...
    return new BinaryExpression(
        merge(n1.left(), n2.left()), merge(n1.right(), n2.right()), n1.operator());
  }
  ...
  public Expression merge(ClassInstantiationExpression n1, ClassInstantiationExpression n2) {
    ...
    MergeException._assert(
        ...
        errorPrefix
            + "different generic argument sizes: "
            + n1.typeArguments().size()
            + " and "
            + n2.typeArguments().size(),
        n1,
        n2);
    for (int i = 0; i < n1.typeArguments().size(); i++) {
      MergeException._assert(
          ...
          errorPrefix
              ...
              + ppv.visit(n1.typeArguments().get(i))
              + " and "
              + ppv.visit(n2.typeArguments().get(i)),
          n1,
          n2);
    }
    MergeException._assert(
    ...
    return new ClassInstantiationExpression(
        new TypeExpression(
            n1.typeExpression().name(),
            n1.typeExpression().worldArguments(),
            n1.typeExpression().typeArguments()),
        arguments,
        n1.typeArguments());
  }
  ...
  public Expression merge(EnumCaseInstantiationExpression n1, EnumCaseInstantiationExpression n2) {
    ...
    MergeException._assert(
        ...
        errorPrefix + "enums with different names: " + n1.name() + " and " + n2.name(),
        n1,
        n2);
    MergeException._assert(
  ...
  public Expression merge(EnclosedExpression n1, EnclosedExpression n2) {
    return new EnclosedExpression(merge(n1.nestedExpression(), n2.nestedExpression()));
  }
  ...
  public Expression merge(FieldAccessExpression n1, FieldAccessExpression n2) {
    MergeException._assert(
        n1.name().equals(n2.name()),
        "Cannot merge field access expressions due to different fields: "
            + n1.name()
            + " and "
            + n2.name(),
        n1,
  ...
  public Expression merge(StaticAccessExpression n1, StaticAccessExpression n2) {
    MergeException._assert(
        ...
        "Cannot merge static access expressions due to different types: "
            + ppv.visit(n1.typeExpression())
            + " and "
            + ppv.visit(n2.typeExpression()),
        n1,
        n2);
  ...
  public Expression merge(MethodCallExpression n1, MethodCallExpression n2) {
    ...
    MergeException._assert(
        n1.name().equals(n2.name()),
        errorPrefix + "different names: " + n1.name() + " and " + n2.name(),
        n1,
        n2);
    MergeException._assert(
        n1.typeArguments().size() == n2.typeArguments().size(),
  ...
  public Expression merge(LiteralExpression n1, LiteralExpression n2) {
    ...
    MergeException._assert(
        ...
        errorPrefix + "different values: " + n1.content() + " and " + n2.content(),
        n1,
        n2);
    return n1;
  }
  ...
  public Expression merge(ScopedExpression n1, ScopedExpression n2) {
    return new ScopedExpression(
        merge(n1.scope(), n2.scope()), merge(n1.scopedExpression(), n2.scopedExpression()));
  }
}
