...
public class StatementsMerger extends AbstractMerger<Statement> {
  ...
  public static Statement merge(List<Statement> n) {
    if (n.size() < 2) {
      ...
      return new StatementsMerger()
          .merge(StatementsUnitNormaliser.visitStatement(n.get(0)), merge(n.subList(1, n.size())));
    }
  }
  ...
  public Statement merge(VariableDeclarationStatement n1, VariableDeclarationStatement n2) {
    ...
    for (int i = 0; i < n1.variables().size(); i++) {
      v.add(
          VariableDeclarationMerger.mergeVariableDeclarations(
              n1.variables().get(i), n2.variables().get(i)));
    }
  ...
  private static Pair<
          Map<SwitchArgument<?>, Statement>, Optional<Map.Entry<SwitchArgument<?>, Statement>>>
      checkSingleDefaultCase(SwitchStatement s) {
  ...
  public TryCatchStatement merge(TryCatchStatement n1, TryCatchStatement n2) {
    MergeException._assert(
        n1.catches().size() == n2.catches().size(),
    ...
    for (int i = 0; i < n1.catches().size(); i++) {
      MergeException._assert(
          ...
          "Cannot merge try-catch statements with non-corresponding catch clauses: ",
          n1,
          n2);
      mergedCatches.add(
          new Pair<>(
    ...
    return new TryCatchStatement(
        merge(n1.body(), n2.body()), mergedCatches, merge(n1.continuation(), n2.continuation()));
  }
