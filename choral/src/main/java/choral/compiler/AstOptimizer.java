...
public class AstOptimizer implements ChoralVisitor {
  ...
  public String visitQualifiedName(ChoralParser.QualifiedNameContext qn) {
    debugInfo();
    return ifPresent(qn.qualifiedName()).applyOrElse(q -> visitQualifiedName(q) + ".", String::new)
  ...
  public Annotation visitAnnotation(ChoralParser.AnnotationContext ac) {
    ...
    if (isPresent(ac.literal())) {
      ...
      annotationValues.put(
          new Name("value", getPosition(ac.literal())), visitLiteral(ac.literal(), null));
    ...
    return new Annotation(getName(ac.Identifier()), annotationValues, getPosition(ac));
  }
  ...
  public Map<Name, LiteralExpression> visitAnnotationValues(
    ...
    vm.put(getName(avc.Identifier()), visitLiteral(avc.literal(), null));
  ...
  public Void visitInterfaceDeclaration(ChoralParser.InterfaceDeclarationContext id) {
    ...
    // Retrieve the world parameters
    List<FormalWorldParameter> worldParameters =
        ifPresent(id.worldParameters())
            .applyOrElse(this::visitWorldParameters, Collections::emptyList);

    // Retrieve the type parameters (with unbound worlds)
    List<FormalTypeParameter> typeParameters =
        ifPresent(id.typeParameters())
            .applyOrElse(this::visitTypeParameters, Collections::emptyList);

    // Retrieve the interfaces it extends
    List<TypeExpression> superTypes =
        ifPresent(id.extendsInterfaces())
  ...
  public List<WorldArgument> visitWorldArguments(ChoralParser.WorldArgumentsContext wps) {
    debugInfo();
    return ifPresent(wps.worldArgument())
        .applyOrElse(
  ...
  @Override
  public List<WorldArgument> visitWorldArgumentList(ChoralParser.WorldArgumentListContext wpl) {
    debugInfo();
    return wpl.worldArgument().stream().map(this::visitWorldArgument).collect(Collectors.toList());
  }

  @Override
  public WorldArgument visitWorldArgument(ChoralParser.WorldArgumentContext wp) {
    debugInfo();
    return new WorldArgument(getName(wp.Identifier()), getPosition(wp));
  }

  @Override
  public List<FormalWorldParameter> visitWorldParameters(ChoralParser.WorldParametersContext wps) {
    debugInfo();
    return ifPresent(wps.worldParameter())
        .applyOrElse(
  ...
  public FormalWorldParameter visitWorldParameter(ChoralParser.WorldParameterContext wp) {
    debugInfo();
    return new FormalWorldParameter(getName(wp.Identifier()), getPosition(wp));
  }

  @Override
  public List<FormalTypeParameter> visitTypeParameters(ChoralParser.TypeParametersContext tpc) {
    debugInfo();
  ...
  public List<FormalTypeParameter> visitTypeParameterList(
    ...
    debugInfo();
    return tpl.typeParameter().stream().map(this::visitTypeParameter).collect(Collectors.toList());
  }
  ...
  public FormalTypeParameter visitTypeParameter(ChoralParser.TypeParameterContext tp) {
    ...
    return new FormalTypeParameter(
        ...
        ifPresent(tp.typeBound()).applyOrElse(this::visitTypeBound, Collections::emptyList),
        tp.annotation().stream().map(this::visitAnnotation).collect(Collectors.toList()),
        getPosition(tp));
  }
  ...
  public List<TypeExpression> visitTypeBound(ChoralParser.TypeBoundContext tb) {
    debugInfo();
    List<TypeExpression> ftpl =
        tb.additionalBound().stream().map(this::visitAdditionalBound).collect(Collectors.toList());
  ...
  @Override
  public List<TypeExpression> visitTypeArgumentList(ChoralParser.TypeArgumentListContext tal) {
    debugInfo();
    return tal.referenceType().stream().map(this::visitReferenceType).collect(Collectors.toList());
  }
  ...
  public TypeExpression visitReferenceType(ChoralParser.ReferenceTypeContext rt) {
    ...
    List<WorldArgument> worlds =
        ifPresent(rt.worldArguments())
            .applyOrElse(this::visitWorldArguments, Collections::emptyList);
    List<TypeExpression> parameters =
        ifPresent(rt.typeArguments()).applyOrElse(this::visitTypeArguments, Collections::emptyList);
    return new TypeExpression(getName(rt.Identifier()), worlds, parameters, getPosition(rt));
  }
  ...
  @Override
  public List<TypeExpression> visitExtendsInterfaces(ChoralParser.ExtendsInterfacesContext ei) {
    debugInfo();
    return visitInterfaceTypeList(ei.interfaceTypeList());
  }

  @Override
  public List<TypeExpression> visitInterfaceTypeList(ChoralParser.InterfaceTypeListContext itl) {
    debugInfo();
    return itl.referenceType().stream()
        .map(t -> visitReferenceType(t))
        .collect(Collectors.toList());
  }
  ...
  public Class visitClassBody(ChoralParser.ClassBodyContext cb) {
    ...
    List<Node> bodyMembers =
        ifPresent(cb.classBodyDeclaration())
            .applyOrElse(
                el ->
                    el.stream()
    ...
    List<Field> fields =
        bodyMembers.stream()
            ...
            .collect(Collectors.toList());
    List<ClassMethodDefinition> methods =
        bodyMembers.stream()
  ...
  public List<Node> visitClassBodyDeclaration(ChoralParser.ClassBodyDeclarationContext cbd) {
    debugInfo();
    return ifPresent(cbd.classMemberDeclaration())
        .applyOrElse(
  ...
  @Override
  public List<Node> visitClassMemberDeclaration(ChoralParser.ClassMemberDeclarationContext cmd) {
    debugInfo();
    if (cmd.fieldDeclaration() != null) {
      ...
      throw new SyntaxException(getPosition(cmd), "Illegal member declaration.");
    }
  }
  ...
  public List<Node> visitFieldDeclaration(ChoralParser.FieldDeclarationContext fd) {
    ...
    return fd.Identifier().stream()
        .map(
            f ->
                new Field(
                    getName(f),
                    visitReferenceType(fd.referenceType()),
                    fd.annotation().stream()
  ...
  public MethodSignature visitMethodHeader(ChoralParser.MethodHeaderContext mh) {
    ...
    return new MethodSignature(
        ...
        ifPresent(mh.typeParameters())
            .applyOrElse(this::visitTypeParameters, Collections::emptyList),
        ifPresent(mh.formalParameterList())
            .applyOrElse(this::visitFormalParameterList, Collections::emptyList),
  ...
  public TypeExpression visitResult(ChoralParser.ResultContext r) {
    debugInfo();
    return ifPresent(r.referenceType())
        .applyOrElse(
            this::visitReferenceType,
            () ->
                new TypeExpression(
                    new Name("void"), Collections.emptyList(), Collections.emptyList()));
  }
  ...
  public List<FormalMethodParameter> visitFormalParameters(
    ...
    debugInfo();
    return fps.formalParameter().stream()
  ...
  public FormalMethodParameter visitFormalParameter(ChoralParser.FormalParameterContext fp) {
    ...
    return new FormalMethodParameter(
        ...
        fp.annotation().stream().map(this::visitAnnotation).collect(Collectors.toList()),
        getPosition(fp));
  }
  ...
  public ConstructorDefinition visitConstructorDeclaration(
    ...
    return new ConstructorDefinition(
        ...
        b.left(),
        b.right(),
        cd.annotation().stream().map(this::visitAnnotation).collect(Collectors.toList()),
        modifiers,
        getPosition(cd));
  }

  @Override
  public ConstructorModifier visitConstructorModifier(ChoralParser.ConstructorModifierContext ctx) {
  ...
  public ConstructorSignature visitConstructorDeclarator(
    ...
    return new ConstructorSignature(
        ...
        ifPresent(cd.typeParameters())
            .applyOrElse(this::visitTypeParameters, Collections::emptyList),
        ifPresent(cd.formalParameterList())
            .applyOrElse(this::visitFormalParameterList, Collections::emptyList),
        getPosition(cd));
  }
  ...
  public MethodCallExpression visitExplicitConstructorInvocation(
    ...
    return new MethodCallExpression(
        ...
        ifPresent(ctx.argumentList()).apply(this::visitArgumentList).orElse(List.of()),
        ifPresent(ctx.typeArguments()).apply(this::visitTypeArguments).orElse(List.of()),
        getPosition(ctx));
  }

  @Override
  public List<InterfaceMethodDefinition> visitInterfaceBody(ChoralParser.InterfaceBodyContext ib) {
    debugInfo();
    return ib.interfaceMethodDeclaration().stream()
        .map(this::visitInterfaceMethodDeclaration)
        .collect(Collectors.toList());
  }
  ...
  public InterfaceMethodDefinition visitInterfaceMethodDeclaration(
    ...
    debugInfo();
    // Retrieve modifiers
    EnumSet<InterfaceMethodModifier> modifiers = EnumSet.noneOf(InterfaceMethodModifier.class);
  ...
  public List<EnumConstant> visitEnumBody(ChoralParser.EnumBodyContext eb) {
    debugInfo();
    return ifPresent(eb.enumConstantList())
        .applyOrElse(this::visitEnumConstantList, Collections::emptyList);
  }
  ...
  public List<EnumConstant> visitEnumConstantList(ChoralParser.EnumConstantListContext ctx) {
    return ctx.enumConstant().stream().map(this::visitEnumConstant).collect(Collectors.toList());
  }
  ...
  public Statement visitBlock(ChoralParser.BlockContext b) {
    debugInfo();
    return ifPresent(b.blockStatements())
        .applyOrElse(this::visitBlockStatements, () -> new NilStatement(getPosition(b)));
  }
  ...
  public Statement visitBlockStatements(ChoralParser.BlockStatementsContext bss) {
    debugInfo();
    if (isPresent(bss)) {
      List<ChoralParser.BlockStatementContext> statements = new ArrayList<>(bss.blockStatement());
      Collections.reverse(statements);
      Statement chainedStatement =
          new NilStatement(getPosition(bss)); // INFO: no line and column here, using bss
      for (ChoralParser.BlockStatementContext s : statements) {
        chainedStatement = visitBlockStatement(s).cloneWithContinuation(chainedStatement);
      }
  ...
  @Override
  public Statement visitLocalVariableDeclaration(ChoralParser.LocalVariableDeclarationContext lvd) {
    debugInfo();
    TypeExpression type = visitReferenceType(lvd.referenceType());
    List<Annotation> annotations =
        lvd.annotation().stream().map(this::visitAnnotation).collect(Collectors.toList());
    return new VariableDeclarationStatement(
        lvd.variableDeclarator().stream()
            .map(vd -> visitVariableDeclarator(vd, type, annotations))
            .collect(Collectors.toList()),
        null,
        getPosition(lvd));
  }
  ...
  @Override
  public VariableDeclaration visitVariableDeclarator(ChoralParser.VariableDeclaratorContext vd) {
  ...
  public VariableDeclaration visitVariableDeclarator(
    ...
    return new VariableDeclaration(
        ...
        type,
        annotations,
        isPresent(vd.shortCircuitOrExpression())
            ? new AssignExpression(
                ifPresent(vd.chainedExpression())
                    .applyOrElse(
                        ce ->
                            this.visitChainedExpression(
                                ce, visitShortCircuitOrExpression(vd.shortCircuitOrExpression())),
                        () -> visitShortCircuitOrExpression(vd.shortCircuitOrExpression())),
                new FieldAccessExpression(getName(vd.Identifier()), getPosition(vd.Identifier())),
  ...
  @Override
  public ExpressionStatement visitExpressionStatement(ChoralParser.ExpressionStatementContext es) {
    debugInfo();
    return new ExpressionStatement(
        isPresent(es.statementExpression())
            ? visitStatementExpression(es.statementExpression())
            : visitChainedExpression(es.chainedExpression(), visitExpression(es.expression())),
        null,
        getPosition(es));
  }
  ...
  public Expression visitTrailingExpression(ChoralParser.TrailingExpressionContext te) {
    ...
    return ifPresent(te.trailExpression())
        .applyOrElse(
            _e -> new ScopedExpression(e, visitTrailExpression(_e), e.position()), () -> e);
  }
  ...
  public Expression visitFieldAccess(ChoralParser.FieldAccessContext fa) {
    ...
    FieldAccessExpression fieldAccessExpr =
        new FieldAccessExpression(getName(fa.Identifier()), getPosition(fa.Identifier()));
    if (isPresent(fa.primary())) {
      return new ScopedExpression(
          visitPrimary(fa.primary()), fieldAccessExpr, fieldAccessExpr.position());
    } else {
      return ifPresent(fa.expressionName())
          .applyOrElse(
              e ->
                  new ScopedExpression(
                      visitExpressionName(e), fieldAccessExpr, fieldAccessExpr.position()),
              () -> fieldAccessExpr);
    }
  }

  @Override
  public Expression visitFieldAccess_no_primary(ChoralParser.FieldAccess_no_primaryContext fanp) {
    FieldAccessExpression fieldAccessExpr =
        new FieldAccessExpression(getName(fanp.Identifier()), getPosition(fanp.Identifier()));
    return ifPresent(fanp.expressionName())
        .applyOrElse(
            e ->
                new ScopedExpression(
                    visitExpressionName(e), fieldAccessExpr, fieldAccessExpr.position()),
  ...
  public Expression visitTrailExpression(ChoralParser.TrailExpressionContext te) {
    ...
    return ifPresent(te.trailExpression())
        .applyOrElse(
            (t) ->
                new ScopedExpression(e, visitTrailExpression(te.trailExpression()), e.position()),
            () -> e);
  }
  ...
  public Expression visitChainedExpression(
    ...
    List<Pair<Expression, ? extends InvocationExpression>> invocationChain =
        ce.chainedInvocation().stream()
            .map(this::visitChainedInvocation)
            .collect(Collectors.toList());
  ...
  public Pair<Expression, ? extends InvocationExpression> visitChainedInvocation(
    ...
    if (isPresent(ci.chainedMethodInvocation())) {
      ...
      return new Pair<>(null, visitChainedClassInstanceCreation(ci.chainedClassInstanceCreation()));
    }
  }
  ...
  public Pair<Expression, MethodCallExpression> visitChainedMethodInvocation(
    ...
    if (isPresent(mi.THIS())) {
      ...
      expression = new FieldAccessExpression(getName(identifiers.remove(0)), getPosition(mi));
    }
  ...
  public Pair<Expression, MethodCallExpression> visitChainedStaticMethodInvocation(
    ...
    List<Name> identifiers =
        smi.Identifier().stream().map(this::getName).collect(Collectors.toList());
    MethodCallExpression mc =
        new MethodCallExpression(
            identifiers.remove(identifiers.size() - 1),
            null,
            ifPresent(smi.typeArguments())
                .apply(this::visitTypeArguments)
                .orElse(Collections.emptyList()),
            getPosition(smi));
    ...
    for (Name identifier : identifiers) {
      e =
          new ScopedExpression(
              e, new FieldAccessExpression(identifier, identifier.position()), getPosition(smi));
    }
  ...
  public StaticAccessExpression visitStaticGenericAccess(
    ...
    return new StaticAccessExpression(
        new TypeExpression(
            ...
            ifPresent(sga.worldArguments())
                .applyOrElse(this::visitWorldArguments, Collections::emptyList),
            ifPresent(sga.typeArguments())
                .applyOrElse(this::visitTypeArguments, Collections::emptyList),
            getPosition(sga)),
        getPosition(sga));
  }
  ...
  public MethodCallExpression visitMethodInvocation(ChoralParser.MethodInvocationContext mi) {
    ...
    return new MethodCallExpression(
        ...
        ifPresent(mi.argumentList()).applyOrElse(this::visitArgumentList, Collections::emptyList),
        ifPresent(mi.typeArguments())
            .apply(this::visitTypeArguments)
            .orElse(Collections.emptyList()),
        getPosition(mi));
  ...
  public List<Expression> visitArgumentList(ChoralParser.ArgumentListContext al) {
    ...
    if (isPresent(al.worldArgumentList())) {
      return visitWorldArgumentList(al.worldArgumentList()).stream()
          .map(w -> visitLiteral(al.literal(), w))
          .collect(Collectors.toList());
    } else {
      return al.expression().stream().map(this::visitExpression).collect(Collectors.toList());
    }
  }
  ...
  public ReturnStatement visitReturnStatement(ChoralParser.ReturnStatementContext rs) {
    ...
    return new ReturnStatement(
        isPresent(rs.chainedExpression())
            ? visitChainedExpression(rs.chainedExpression(), visitExpression(rs.expression()))
  ...
  public Expression visitLeftHandSide(ChoralParser.LeftHandSideContext lhs) {
    ...
    return ifPresent(lhs.expressionName())
        .applyOrElse(this::visitExpressionName, () -> visitFieldAccess(lhs.fieldAccess()));
  }
  ...
  public Expression visitExpression(ChoralParser.ExpressionContext e) {
    ...
    return ifPresent(e.shortCircuitOrExpression())
        .applyOrElse(this::visitShortCircuitOrExpression, () -> visitAssignment(e.assignment()));
  }
  ...
  @Override
  public Expression visitMultiplicativeExpression(ChoralParser.MultiplicativeExpressionContext me) {
    debugInfo();
  ...
  public Expression visitUnaryExpression(ChoralParser.UnaryExpressionContext ue) {
    ...
    if (isPresent(ue.primary())) {
      ...
      return new NotExpression(visitUnaryExpression(ue.unaryExpression()), getPosition(ue));
    }
  }
  ...
  public Expression visitPrimary(ChoralParser.PrimaryContext p) {
    ...
    if (isPresent(p.NULL())) {
      return new NullExpression(visitWorldArguments(p.worldArguments()), getPosition(p));
  ...
  public IfStatement visitIfThenStatement(ChoralParser.IfThenStatementContext its) {
    ...
    return new IfStatement(
        visitExpression(its.expression()),
        visitBlock(its.block()),
        new NilStatement(getPosition(its.getStop())),
        null,
        getPosition(its));
  }
  ...
  public Map<SwitchArgument<?>, Statement> visitSwitchBlock(ChoralParser.SwitchBlockContext sbc) {
    debugInfo();
  ...
  public List<SwitchArgument> visitSwitchArgs(ChoralParser.SwitchArgsContext sac) {
    ...
    if (isPresent(sac.literal())) {
      l.add(
          new SwitchArgument.SwitchArgumentLiteral(
              visitLiteral(
                  sac.literal(),
                  new WorldArgument(
  ...
  public TryCatchStatement visitTryCatchStatement(ChoralParser.TryCatchStatementContext tcs) {
    ...
    return new TryCatchStatement(
        ...
        tcs.catchBlock().stream().map(this::visitCatchBlock).collect(Collectors.toList()),
        null,
        getPosition(tcs));
  }

  @Override
  public Pair<VariableDeclaration, Statement> visitCatchBlock(ChoralParser.CatchBlockContext cb) {
    debugInfo();
    return new Pair<>(
        new VariableDeclaration(
            getName(cb.formalParameter().Identifier()),
            visitReferenceType(cb.formalParameter().referenceType()),
            cb.formalParameter().annotation().stream()
                .map(this::visitAnnotation)
                .collect(Collectors.toList()),
            null,
            getPosition(cb)),
  ...
  public LiteralExpression visitLiteral(ChoralParser.LiteralContext l, WorldArgument w) {
    if (isPresent(l.BooleanLiteral())) {
      ...
      return new LiteralExpression.StringLiteralExpression(
          l.StringLiteral().getText(), w, getPosition(l));
    } else {
      throw new SyntaxException(getPosition(l), "Unrecognized literal: '" + l.getText() + "'");
    }
  }
  ...
  public Expression visitExpressionName(ChoralParser.ExpressionNameContext en) {
    debugInfo();
    FieldAccessExpression fa = new FieldAccessExpression(getName(en.Identifier()), getPosition(en));
    return ifPresent(en.ambiguousName())
        .applyOrElse(
            (an) -> new ScopedExpression(visitAmbiguousName(an), fa, getPosition(en)), () -> fa);
  }
  ...
  public Expression visitAmbiguousName(ChoralParser.AmbiguousNameContext an) {
    debugInfo();
    FieldAccessExpression fa = new FieldAccessExpression(getName(an.Identifier()), getPosition(an));
    return ifPresent(an.ambiguousName())
        .applyOrElse(
            (_an) -> new ScopedExpression(visitAmbiguousName(_an), fa, getPosition(an)), () -> fa);
  }
  ...
  public Object visitErrorNode(ErrorNode errorNode) {
    new ParseException("Parsing Error " + errorNode.getText(), errorNode.getSourceInterval().a)
        .printStackTrace();
    debugInfo();
    return null;
  }
    ...
    if (debug) {
      ...
      if (lastMethod.equals(currentMethod)) {
        ...
        if (!debugExcludeMethods.contains(lastMethod)) {
          System.err.println(lastMethod + ((cmCounter > 1) ? (" x " + cmCounter) : ""));
        }
