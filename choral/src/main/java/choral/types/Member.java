...
//				context.declarationPackage() == this.declarationContext().declarationPackage() )));
		...
		boolean sameType = this.declarationContext().typeConstructor() == context.typeConstructor();
		return this.declarationContext == context
				|| this.isPublic()
				|| this.isProtected()
				|| ( this.isPackagePrivate() && samePackage )
				|| ( this.isPrivate() && sameType );
	}
	...
	public final boolean isAccessibleFrom( GroundTypeParameter context ) {
		...
		boolean samePackage = this.declarationContext().declarationPackage() == contextPackage;
