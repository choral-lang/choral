...
public abstract class HigherClassOrInterface extends HigherReferenceType
  ...
  public HigherClassOrInterface(
    ...
    this(declarationContext, modifiers, identifier, worldsParameters, typeParameters, null, true);
  }
  ...
  private static final EnumSet<Modifier> legalOuterModifiers = EnumSet.of(PUBLIC, ABSTRACT, FINAL);
  ...
  private class Lambda extends HigherReferenceType {
    Lambda(List<? extends HigherReferenceType> typeArgs) {
      super(
          HigherClassOrInterface.this.universe(),
          World.freshWorlds(
              HigherClassOrInterface.this.universe(),
              HigherClassOrInterface.this.worldParameters.size(),
              "X"));
    ...
    public boolean equals(Object obj) {
      if (obj instanceof Lambda) {
        ...
        return this.isSameKind(other)
            && this.applyTo(this.worldParameters)
                .isEquivalentTo(other.applyTo(this.worldParameters));
      }
      return false;
    }
  }
  ...
  public final Kind kind() {
    return Kind.getAtTower(
        worldParameters().size(),
        Kind.getTower(
            typeParameters.stream().map(HigherTypeParameter::kind).collect(Collectors.toList()),
            Kind.getStar()));
  }
  ...
  public abstract class Definition extends HigherReferenceType.Definition
        ...
        allExtendedInterfaces =
            Stream.concat(
                    ...
                    extendedClassesOrInterfaces()
                        .flatMap(GroundClassOrInterface::allExtendedInterfaces))
    ...
    public void addExtendedInterface(GroundInterface type) {
      ...
      if (extendedInterfaces().anyMatch(x -> x.isEquivalentTo(type))) {
        throw new StaticVerificationException("illegal inheritance, '" + type + "' is repeated");
      }
    ...
    protected boolean isEquivalentTo_relaxed(GroundDataType type) {
      if (type == this) {
        ...
        Proxy other = (Proxy) type;
        if (other.definition() != this) return false;
    ...
    protected boolean isSubtypeOf(GroundDataType type, boolean strict) {
      return (!strict && isEquivalentTo(type))
          ...
          || (type.isEquivalentTo(universe().topReferenceType(worldArguments())));
    }
    ...
    protected boolean isSubtypeOf_relaxed(GroundDataType type, boolean strict) {
      return (!strict && isEquivalentTo_relaxed(type))
          ...
          || (type.isEquivalentTo_relaxed(universe().topReferenceType(worldArguments())));
    }
    ...
    public void finaliseInterface() {
      assert (isInheritanceFinalised()
          && extendedClassesOrInterfaces().allMatch(GroundReferenceType::isInterfaceFinalised));
    ...
    /**
     * (JLS 8.3) Inherit fields from direct superclasses and superinterfaces. A field is inherited
     * if it is accessible from this type and not hidden by a field declared in this type with the
     * same name.
     */
    private void inheritFields() {
      extendedClassesOrInterfaces()
          ...
          .filter(
              x ->
                  x.isAccessibleFrom(this)
                      && declaredFields().noneMatch(y -> x.identifier().equals(y.identifier())))
          .forEach(inheritedFields::add);
    }

    /**
     * Collect inherited methods, applying JLS rules for overriding, override-equivalent resolution,
     * return-type compatibility, and abstract obligations (JLS 8.4.8 for classes, 9.4.1 for
     * interfaces).
     */
      ...
      if (this instanceof GroundClass gc) {
            ...
            .ifPresent(
                superclass ->
                    superclass
                        ...
                        .forEach(m -> collectCandidate(m, candidates, implementedByDeclared)));

        List<Member.HigherMethod> concreteSuperclassMethods = new ArrayList<>(candidates);
            ...
            .ifPresent(
                superclass ->
                    superclass
                        ...
                        .forEach(
                            m ->
                                collectCandidateIfNotSatisfied(
                                    m,
                                    candidates,
                                    concreteSuperclassMethods,
        ...
        extendedInterfaces()
            ...
            .forEach(
                m ->
                    collectCandidateIfNotSatisfied(
                        m, candidates, concreteSuperclassMethods, implementedByDeclared));
    ...
    /**
     * Add {@code methodToInherit} as a candidate unless a declared method overrides it (JLS
     * 8.4.8.1–.3). Records an entry in {@code implementedByDeclared} when the overriding declared
     * method is concrete. Erasure-only overrides keep the parent around for bridge generation.
     */
        ...
        if (methodToInherit.sameErasureAs(declaredMethod)) {
          ...
          throw new StaticVerificationException(
              "method '"
                  ...
                  + methodToInherit
                  + "' in '"
                  + methodToInherit.declarationContext()
    ...
    /**
     * (JLS 8.4.8 (d)) Variant of {@link #collectCandidate} that skips parent methods already
     * satisfied by a concrete superclass method.
     */
    ...
    /**
     * Checks whether an already-collected candidate covers a method that needs inheritance only for
     * bridge purposes (erasure-only override).
     */
    private boolean isAlreadyCoveredByCandidate(
        Member.HigherMethod methodToInherit, List<Member.HigherMethod> candidates) {
    ...
    /**
     * Resolve override-equivalent pairs in {@code candidates}, removing duplicates and losers by
     * specificity, diamond identity, or default-method resolution. Throws on unresolvable conflicts
     * (JLS 8.4.8.4 / 9.4.1.3). Modifies {@code candidates} in place.
     */
    private void resolveOverrideEquivalentGroups(List<Member.HigherMethod> candidates) {
     ...
     *
     * @return negative if {@code m} should be removed (earlier wins), positive if {@code earlier}
     *     should be removed (m wins), zero if both should be kept (e.g., both abstract).
     * @throws StaticVerificationException on unresolvable conflict
     */
    private int resolveOverrideEquivalentPair(Member.HigherMethod m, Member.HigherMethod earlier) {
    ...
    /**
     * Checks whether there exists an abstract method declared in a superclass of this class (and
     * inherited by this class) that is override-equivalent with the given method. This only applies
     ...
     * return-type-substitutable for every other. (JLS 8.1.5 / 8.4.3.1) A concrete class must
     * implement every inherited abstract method.
     */
    private void checkInheritedMethodCompatibility(
        List<Member.HigherMethod> candidates, Set<Member.HigherMethod> implementedByDeclared) {
    ...
    /**
     * For a concrete class inheriting an abstract method, checks whether an already-inherited
     * concrete method satisfies the implementation requirement. Throws if no implementation is
     * found.
     */
    private void checkAbstractImplementation(
        Member.HigherMethod abstractMethod, List<Member.HigherMethod> candidates) {
      ...
      throw new StaticVerificationException(
          "'"
              ...
              + abstractMethod
              + "' in '"
              + abstractMethod.declarationContext()
              + "'");
    }

    /**
     * (JLS 8.1.5 / 8.4.3.1) An inaccessible abstract superclass method can never be overridden (JLS
     * 8.4.8.1 requires accessibility), so any such method is always an error for a concrete
     * subclass.
     */
    ...
    /**
     * True if a common ancestor interface of {@code context1} and {@code context2} declares a
     * default method with {@code method}'s signature (diamond inheritance).
     */
    private boolean shareCommonDefaultOrigin(
        Member.HigherMethod method, GroundReferenceType context1, GroundReferenceType context2) {
      ...
      return c1.allExtendedInterfaces()
          .anyMatch(
              i -> {
    ...
    private void checkOverrideRequirementsOrThrow(
      ...
      // (8.4.8.3) JLS says we should issue a warning if child is not a subtype of parent; we skip
      // that check.
      ...
      if (child.isPrivate()
          || (parent.isPublic() && !child.isPublic())
      ...
      for (Member.HigherMethod x : declaredMethods) {
        if (x.sameErasureAs(method)) {
          if (x.sameSignatureAs(method)) {
            throw new StaticVerificationException(
                "method '" + method + "' is already defined in '" + typeConstructor() + "'");
          } else {
            throw new StaticVerificationException(
                "method '"
                    + method
  ...
  protected abstract class Proxy extends HigherReferenceType.Proxy
    ...
    public final List<? extends HigherReferenceType> typeArguments() {
      return typeConstructor().typeParameters().stream()
    ...
    @Override
    public final Stream<GroundInterface> extendedInterfaces() {
      return definition().extendedInterfaces().map(x -> x.applySubstitution(substitution()));
    }

    @Override
    public final Stream<GroundInterface> allExtendedInterfaces() {
      return definition().allExtendedInterfaces().map(x -> x.applySubstitution(substitution()));
    }
