...
public class Universe {
  ...
  private static final HashMap<String, PrimitiveTypeTag> primitiveTypesConversionMap =
      new HashMap<>(8);
  private static final Map<SpecialTypeTag, PrimitiveTypeTag> unboxingMap =
      new EnumMap<>(SpecialTypeTag.class);
  ...
  private final Map<PrimitiveTypeTag, HigherPrimitiveDataType> primitiveTypes =
      new EnumMap<>(PrimitiveTypeTag.class);
  ...
  private final Map<SpecialTypeTag, HigherClassOrInterface> specialClasses =
      new EnumMap<>(SpecialTypeTag.class);
  ...
  public HigherClassOrInterface specialType(SpecialTypeTag key) {
    ...
    if (result == null) {
      throw new StaticVerificationException(
          "Unknown class '" + key.qualifiedName + "', missing a header?");
    }
    return result;
  }
  ...
  HigherClass topReferenceType(int worlds) {
    ...
    if (worlds == 1) {
      ...
      if (t == null) {
        ...
        t =
            new HigherClass(
                rootPackage(), EnumSet.of(PUBLIC, ABSTRACT), "Any", ws, List.of(), false);
  ...
  private final class HigherNullType extends HigherReferenceType {
    ...
    public String toString() {
      return this.worldParameters.stream()
              .map(World::toString)
    ...
    final class GroundNullType extends TypeBase implements GroundReferenceType {
      ...
      public GroundNullType applySubstitution(Substitution substitution) {
        return typeConstructor()
            .applyTo(worldArguments().stream().map(substitution::get).collect(Collectors.toList()));
      }
  ...
  private final GroundDataTypeOrVoid voidType =
      new GroundDataTypeOrVoid() {
        @Override
