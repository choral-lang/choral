...
public class FilterSourceUnits {
   ...
   *
   * <p>Takes a Collection of CompilationUnits and a symbol and returns the CompilationUnit matching
   * the symbol as CompilationUnits matching any imports.
   *
   * <p>Say you have the following filesystem:
   *
   * <pre>
   * src
   * ├─ proj1
   ...
   * the source folder to src. The way choral collects files to project is by finding all ".ch"
   * files inside the source folder, and parse and type all of them, then only projecting your
   * desired file (here, Project1.ch).
   *
   * <p>This means, however, that Project2.ch would also be typed (despite not being imported by
   * Project1.ch), and if it contains errors it would cause the compiler to throw an error and not
   * project Project1.ch.
   *
   * <p>With FilterSourceUnits we can filter out those CompilationUnits that our main
   * CompilationUnit (the one matching symbol) doesn't need.
   */
  public static Collection<CompilationUnit> filterSourceUnits(
      Collection<CompilationUnit> allSourceUnits, String symbol) {
    ...
    while (!tempSources.isEmpty()) {
      ...
      for (ImportDeclaration imp : source.imports()) { // Iterate through all imports
        ...
        for (CompilationUnit potentialSource : allSourceUnits) { // Iterate through all source units
          if (potentialSource.packageDeclaration().isPresent()
              && potentialSource
                  .packageDeclaration()
                  .get()
                  .equals(imp.name().substring(0, lastDot))) {
            ...
            if (imp.isOnDemand()) { // a "*" import
              ...
              //     if( !tempSources.contains( potentialSource ) && !sourceUnits.contains(
              // potentialSource ) )
              //         tempSources.add( potentialSource );
              // }
            }
