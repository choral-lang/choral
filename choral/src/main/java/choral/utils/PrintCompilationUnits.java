...
public class PrintCompilationUnits {
	...
	public static void printCompilationUnits( Collection< CompilationUnit > compilationUnits ) {
		compilationUnits.stream().forEach( compilationUnit -> {
			String string =
					"\u001B[34mSource file:\u001B[0m " + compilationUnit.position().sourceFile();
			printWithIndentation( string, 1 );
			string = blue( "Package: " ) + ( compilationUnit.packageDeclaration().isPresent() ?
					compilationUnit.packageDeclaration().get() : "None" );
			printWithIndentation( string, 2 );
			string = blue( "Imports: " ) + compilationUnit.imports().stream().map( im -> im.name() )
					.collect( Collectors.toList() );
			printWithIndentation( string, 2 );
			string = blue( "Interfaces: " ) + compilationUnit.interfaces().stream()
					.map( in -> in.name() ).collect( Collectors.toList() );
			printWithIndentation( string, 2 );
			string = blue( "Classes: " ) + compilationUnit.classes().stream().map( cl -> cl.name() )
					.collect( Collectors.toList() );
			printWithIndentation( string, 2 );
			string = blue( "Class methods: " ) + compilationUnit
					.classes().stream().map( cl -> cl.methods().stream()
							.map( meth -> meth.toString() ).collect( Collectors.toList() ) )
					.collect( Collectors.toList() );
			printWithIndentation( string, 3 );
			string = blue( "Enums: " ) + compilationUnit.enums().stream().map( en -> en.name() )
					.collect( Collectors.toList() );
			printWithIndentation( string, 2 );
		} );
	}
		...
		units.forEach( sourceUnit -> {
			...
			sourceUnit.classes().forEach( cls -> {
				...
				cls.methods().forEach( method -> {
					// method 
					System.out.println( "Dependencies for method "
							+ blue( method.signature().typeAnnotation().get().toString() ) );
					if( !method.signature().typeAnnotation().get().worldDependencies().isEmpty() ) {
						method.signature().typeAnnotation().get().worldDependencies()
								.forEach( ( world, depenList ) -> {
									// world dependencies
									System.out.println( "\tRole " + world + " needs" );
									depenList.forEach( dependency -> System.out
											.println( "\t\t" + dependency.left() + " of type "
													+ ( (GroundDataType) dependency.left()
															.typeAnnotation().get() ).toString()
													+ " at " + dependency.left().position() ) );
								} );
					} else {
						System.out.println( "\tNone" );
					}
					System.out.println( "Channels for method "
							+ blue( method.signature().typeAnnotation().get().toString() ) );
					if( !method.signature().typeAnnotation().get().channels().isEmpty() ) {
						method.signature().typeAnnotation().get().channels()
