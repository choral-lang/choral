    ...
    compilationUnits.stream()
        .forEach(
            compilationUnit -> {
              ...
              string =
                  blue("Package: ")
                      + (compilationUnit.packageDeclaration().isPresent()
                          ? compilationUnit.packageDeclaration().get()
                          : "None");
              ...
              string =
                  blue("Interfaces: ")
                      + compilationUnit.interfaces().stream()
                          .map(in -> in.name())
                          .collect(Collectors.toList());
              printWithIndentation(string, 2);
              string =
                  ...
                  blue("Class methods: ")
                      + compilationUnit.classes().stream()
                          .map(
                              cl ->
                                  cl.methods().stream()
                                      .map(meth -> meth.toString())
                                      .collect(Collectors.toList()))
                          .collect(Collectors.toList());
              printWithIndentation(string, 3);
              string =
                  blue("Enums: ")
                      + compilationUnit.enums().stream()
                          .map(en -> en.name())
                          .collect(Collectors.toList());
              printWithIndentation(string, 2);
            });
