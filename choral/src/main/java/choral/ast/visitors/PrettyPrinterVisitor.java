...
public class PrettyPrinterVisitor implements ChoralVisitorInterface<String> {
  ...
  public String visit(CompilationUnit n) {
    return (n.packageDeclaration().isPresent()
  ...
  protected static <E extends java.lang.Enum<E>> String visitModifiers(EnumSet<E> modifiers) {
    if (modifiers.size() == 0) {
      return "";
    } else {
  ...
  public String visit(Interface n) {
    ...
    String template =
        "${annotations}$modifiers$interface $type$extends {"
            + NEWLINE
            + "#if( $!{methods.trim()} != '')$methods"
            + NEWLINE
            + "#end"
            + "}"
            + NEWLINE;
  ...
  public String visit(Class n) {
    ...
    m.put(
        "extends",
        ifPresent(n.superClass().orElse(null))
            .applyOrElse(t -> " " + EXTENDS + " " + visit(t), String::new));
    m.put(
        "implements",
        n.implementsInterfaces().isEmpty()
            ? ""
            : " " + IMPLEMENTS + " " + visitAndCollect(n.implementsInterfaces(), COMMA));
    m.put("fields", indent(visitAndCollect(n.fields(), SEMICOLON + NEWLINE, SEMICOLON + NEWLINE)));
    m.put("methods", indent(visitAndCollect(n.methods(), _2NEWLINE, _2NEWLINE)));
    m.put("constructors", indent(visitAndCollect(n.constructors(), _2NEWLINE, _2NEWLINE)));
    m.put("modifiers", visitModifiers(n.modifiers()));
    m.put("annotations", visitAndCollect(n.annotations(), NEWLINE, NEWLINE));

    String template =
        "${annotations}$modifiers$class $type$extends$implements {"
            + NEWLINE
            + "#if( $!{fields.trim()} != '' )$fields"
            + _2NEWLINE
            + "#end"
            + "#if( $!{constructors.trim()} != '' )$constructors"
            + _2NEWLINE
            + "#end"
            + "#if( $!{methods.trim()} != '')$methods"
            + _2NEWLINE
  ...
  public String visit(Enum n) {
    ...
    m.put("annotations", visitAndCollect(n.annotations(), NEWLINE, NEWLINE));
    String template =
        "${annotations}$modifiers$enum $type {"
            + NEWLINE
            + "#if( $!{cases.trim()} != '' )$cases"
            + NEWLINE
            + "#end"
  ...
  public String visit(IfStatement n) {
    ...
    template.append("if( $condition ){" + NEWLINE + "$ifBranch" + NEWLINE + "}");
    ...
    if (!(n.elseBranch() instanceof NilStatement)) {
      ...
      template.append(" else { " + NEWLINE + "$elseBranch" + NEWLINE + "}");
    }
    return Utils.createVelocityTemplate(template.toString()).render(m) + getContinuation(n, "");
  }
  ...
  public String visit(TryCatchStatement n) {
    HashMap<String, Object> m = new HashMap<>();
    String template = "try {" + NEWLINE + "$body" + NEWLINE + "}" + NEWLINE + "$catches";
  ...
  public String visit(MethodCallExpression n) {
    return (n.typeArguments().isEmpty()
            ? ""
            : "< " + visitAndCollect(n.typeArguments(), SPACED_COMMA) + " >")
        + visit(n.name())
        + (n.arguments().isEmpty()
    ...
    String variables =
        n.variables().stream()
            .map(
                v ->
                    visit(v.name())
                        + v.initializer().map(e -> ASSIGN + visit(e.value())).orElse(""))
  ...
  public String visit(LiteralExpression.BooleanLiteralExpression n) {
    return n.content().toString()
        + ifPresent(n.world()).applyOrElse(w -> "@" + w.name().identifier(), String::new);
  }
  ...
  public String visit(ClassMethodDefinition n) {
    ...
    if (n.body().isPresent()) {
      ...
      template = "${annotations}$modifiers$signature {" + NEWLINE + "$body" + NEWLINE + "}";
    } else {
  ...
  public String visit(FormalTypeParameter n) {
    ...
    if (!n.upperBound().isEmpty()) {
      s.append(' ')
          .append(EXTENDS)
          .append(' ')
          .append(n.upperBound().stream().map(this::visit).collect(Collectors.joining(AMPERSAND)));
    }
    return s.toString();
  }
  ...
  protected final String getContinuation(Statement s, String closure) {
    return closure
        + ifPresent(s.continuation()).applyOrElse((e) -> NEWLINE + visit(e), String::new);
  }
