...
public class ChoralUnit {
  ...
  public static void main(String[] args) {
    ...
    ClassInfoList classes =
        scanResult
            .getClassesWithAnnotation(Choreography.class.getName())
            .filter(
                c ->
                    c.getAnnotationInfo(Choreography.class.getName())
                        .getParameterValues()
                        .getValue("name")
                        .equals(target));
    ...
    MethodInfoList methods =
        classes.get(0).getMethodInfo().filter(m -> m.hasAnnotation(Test.class.getName()));
    ...
    for (MethodInfo method : methods) {
                  ...
                  try {
                    ...
                    if (Modifier.isStatic(classMethod.getModifiers())) {
                      Class.forName(cls.getName())
                          ...
                          .invoke(Class.forName(cls.getName()));
                    } else {
                      throw new ChoralUnitException(
                          "Test method "
                              + method.getName()
      ...
      if (!threadExceptions.isEmpty()) throw new RuntimeException(threadExceptions.get(0));
    }
  }
}
