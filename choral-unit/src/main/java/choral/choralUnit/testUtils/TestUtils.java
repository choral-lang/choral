...
public class TestUtils {
  ...
  private static final Map<String, Pair<SymChannel_A<Object>, SymChannel_B<Object>>> channels =
      new HashMap<>();
  private static final Map<String, Pair<TLSChannel_A<Object>, TLSChannel_B<Object>>> tlsChannels =
      new HashMap<>();
  ...
  public static Pair<SerializerChannel_A, SerializerChannel_B> newSocketChannel(int server_port)
      throws ExecutionException, InterruptedException {
    server_port =
        server_port == 0 ? ThreadLocalRandom.current().nextInt(10000, 65000) : server_port;
    ServerSocketByteChannel serverListener = ServerSocketByteChannel.at("localhost", server_port);
    ...
    if (!tlsChannels.containsKey(id)) {
      try {
        ...
        tlsChannels.put(
            ...
            new Pair<>(
                new TLSChannel_A<>(
                    new TSLByteChannel_A(localChannel1, sslContext), KryoSerializer.getInstance()),
                new TLSChannel_B<>(
                    new TSLByteChannel_B(localChannel2, sslContext),
                    KryoSerializer.getInstance())));
      } catch (IOException
