...
public class Karatsuba_C {
	public static Unit multiply(
			Unit n1, Unit n2, Unit ch_AB, SymChannel_B< Object > ch_BC, SymChannel_A< Object > ch_CA
	) {
	...
	public static Unit multiply( SymChannel_B< Object > ch_BC, SymChannel_A< Object > ch_CA ) {
		{
			switch( ch_CA.< Choice >select( Unit.id ) ) {
				...
				case RECUR -> {
					...
					ch_CA.< Long >com( Karatsuba_A.multiply( ch_CA.< Long >com( Unit.id ),
							ch_CA.< Long >com( Unit.id ), ch_CA, Unit.id, ch_BC ) );
