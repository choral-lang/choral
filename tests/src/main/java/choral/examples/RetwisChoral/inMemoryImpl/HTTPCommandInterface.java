  ...
  public void addContexts() {
    ...
    httpServer.createContext(
        ...
        exchange -> {
          Headers headers = exchange.getRequestHeaders();
          Emitter.Action action =
              new Emitter.Status(headers.getFirst(Emitter.Action.Fields.statusPostID.name()));
          addAction(action);
          handleResponse(exchange);
        });
  }
  ...
  private void handleResponse(HttpExchange exchange) {
    try {
      ...
      exchange.sendResponseHeaders(
          responseMessage.isError() ? 500 : 200, response.getBytes(StandardCharsets.UTF_8).length);
