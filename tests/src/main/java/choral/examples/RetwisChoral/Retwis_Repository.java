...
public class Retwis_Repository {
	...
	public Retwis_Repository(
			Unit chCS, SymChannel_B< Object > chSR, Unit commandInterface,
			DatabaseConnection databaseConnection, Unit sessionManager
	) {
			...
			switch( chSR.< Result >select( Unit.id ) ) {
				case OK -> {
					...
					chSR.< Posts >com( Posts.of( databaseConnection.getFollowers( username ),
							databaseConnection.getFollowed( username ), databaseConnection
									.getPostsPage( username, chSR.< Integer >com( Unit.id ) ) ) );
				}
				case ERROR -> {
	...
	private void mentions() {
		{
			...
			switch( chSR.< Result >select( Unit.id ) ) {
				case OK -> {
					chSR.< Mentions >com( databaseConnection.mentions(
							chSR.< String >com( Unit.id ), chSR.< Boolean >com( Unit.id ) ) );
				}
				case ERROR -> {
	...
	private Unit checkUser( Unit name ) {
		return chSR
				.< Boolean >com( databaseConnection.isUserValid( chSR.< String >com( Unit.id ) ) );
	}

	private Unit checkPost( Unit postID ) {
		return chSR
				.< Boolean >com( databaseConnection.isPostValid( chSR.< String >com( Unit.id ) ) );
	}
