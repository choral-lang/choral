...
public class Retwis_Repository {
        ...
        case OK -> {
          ...
          chSR.<Posts>com(
              Posts.of(
                  ...
                  databaseConnection.getPostsPage(username, chSR.<Integer>com(Unit.id))));
        }
        case ERROR -> {}
        default -> {
          throw new RuntimeException("Received unexpected label from select operation");
                  ...
                  switch (chSR.<Result>select(Unit.id)) {
                    case OK -> {
                      databaseConnection.follow(
                          chSR.<String>com(Unit.id), chSR.<String>com(Unit.id));
                    }
                    case ERROR -> {}
                    default -> {
                      throw new RuntimeException("Received unexpected label from select operation");
  ...
  private void mentions() {
    {
      ...
      switch (chSR.<Result>select(Unit.id)) {
        case OK -> {
          chSR.<Mentions>com(
              databaseConnection.mentions(chSR.<String>com(Unit.id), chSR.<Boolean>com(Unit.id)));
        }
        case ERROR -> {}
        default -> {
          throw new RuntimeException("Received unexpected label from select operation");
  ...
  private Unit checkFollow(Unit name, Unit followTarget) {
    return chSR.<Boolean>com(
        databaseConnection.isFollower(chSR.<String>com(Unit.id), chSR.<String>com(Unit.id)));
  }
}
