...
public class DemoDistributed {
    ...
    executor.submit(
        () -> {
          ...
          while (listener.isOpen()) {
            ...
            try {
              SerializerChannel_B chSR =
                  new SerializerChannel_B(
                      KryoSerializer.getInstance(), new WrapperByteChannel_B(listener.getNext()));
              executor.submit(
                  () -> {
                    try {
    ...
    executor.submit(
        () -> {
          ...
          while (listener.isOpen()) {
            ...
            try {
              SerializerChannel_B chCS =
                  new SerializerChannel_B(
                      KryoSerializer.getInstance(), new WrapperByteChannel_B(listener.getNext()));
              executor.submit(
                  () -> {
                    SerializerChannel_A chSR =
  ...
  public static void main(String[] args) throws InterruptedException, ExecutionException {
    ...
    ServerSocketByteChannel repositoryListener =
        ServerSocketByteChannel.at("localhost", REPOSITORY_PORT);
