...
public class Retwis_Server {
	...
	public Retwis_Server(
			SymChannel_B< Object > chCS, SymChannel_A< Object > chSR, Unit commandInterface,
			Unit databaseConnection, SessionManager sessionManager
	) {
	...
	private void post() {
		...
		if( sessionManager.checkLoggedUser( token ) ) {
			chSR.< Result >select( Result.OK );
			chCS.< Result >select( Result.OK );
			Unit.id( chSR.< String >com( sessionManager.getUsernameFromToken( token ) ),
	...
	private void mentions() {
		...
		if( checkUser( mentionsName ) ) {
			...
			Boolean selfMentions =
					mentionsName.equals( sessionManager.getUsernameFromToken( token ) );
			chCS.< Mentions >com( chSR.< Mentions >com( Unit.id( chSR.< String >com( mentionsName ),
					chSR.< Boolean >com( selfMentions ) ) ) );
		} else {
			chSR.< Result >select( Result.ERROR );
			chCS.< Result >select( Result.ERROR );
		}
