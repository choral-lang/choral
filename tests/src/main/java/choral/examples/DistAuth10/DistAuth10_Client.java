...
public class DistAuth10_Client {
	...
	public DistAuth10_Client(
			TLSChannel_A< Object > ch_Client_IP, Unit ch_Service_IP, Unit ch_s1, Unit ch_s2,
			Unit ch_s3, Unit ch_s4, Unit ch_s5, Unit ch_s6, Unit ch_s7
	) {
		this( ch_Client_IP );
	}
	...
	private String calcHash( String salt, String pwd ) {
		...
		try {
			...
			return Base64_Encoder
					.encodeToString( md.digest( salt_and_pwd.getBytes( StandardCharsets.UTF_8 ) ) );
	...
	public AuthResult_A authenticate( Credentials credentials ) {
		String salt =
				ch_Client_IP.< String >com( ch_Client_IP.< String >com( credentials.username ) );
