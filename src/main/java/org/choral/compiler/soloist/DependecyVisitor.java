...
public class DependecyVisitor implements ChoralVisitorInterface< Void > {
	...
	public DependecyVisitor( List< CompilationUnit > CUs, WorldArgument w ) {
		...
		CUs.forEach( cu -> {
			cu.interfaces().forEach( i -> availableTemplates.put( i.name().identifier(), new AvailableTemplate( i, cu ) ) );
			cu.classes().forEach( c -> availableTemplates.put( c.name().identifier(), new AvailableTemplate( c, cu ) ) );
			cu.enums().forEach( e -> availableTemplates.put( e.name().identifier(), new AvailableTemplate( e, cu ) ) );
		} );
	}
	...
	private Set< Pair< String, Integer > > collectProjectableTemplates( String template ) {
		if( availableTemplates.containsKey( template ) ){
			...
			return projectableTemplates.stream().filter( p -> availableTemplates.containsKey( p.left() ) ).collect( Collectors.toSet() );
		} else{
	...
	private void visitProjectableTemplates() {
		...
		projectableTemplates.stream()
				.filter( p -> !visitedTemplates.contains( p ) ) // we optimise the check to execute only on those not visited yet
				.forEach( p -> {
					if( !availableTemplates.containsKey( p.left() ) ) visitedTemplates.add( p );
				} );
		// we collect the remaining projectable templates
		new HashSet<>( projectableTemplates ) // we take a snapshot of the current projectable templates
	...
	public Void visit( CompilationUnit n ) {
		throw new SoloistProjectorException( "DependencyVisitor should not visit a compilation unit" );
	}

	@Override
	public Void visit( ImportDeclaration n ) {
		throw new SoloistProjectorException( "DependencyVisitor should not visit an import declaration" );
	}
	...
	public Void visit( ClassInstantiationExpression n ) {
		if( n.typeExpression().worldArguments().contains( this.w ) ) {
			projectableTemplates.add( new Pair<>( n.typeExpression().name().identifier(), n.typeExpression().worldArguments().indexOf( this.w ) ) );
	...
	public Void visit( CaseSignature n ) {
		throw new SoloistProjectorException( "DependencyVisitor should not visit a case signature" );
	}
...
//		visit( n.body() );
	...
	public Void visit( TypeExpression n ) {
		if( n.worldArguments().contains( this.w ) ){
			this.projectableTemplates.add( new Pair<>( n.name().identifier(), n.worldArguments().indexOf( this.w ) ) );
		}
		n.typeArguments().forEach( this::visit );
		return null;
	}
	...
	public Void visit( WorldArgument n ) {
		throw new SoloistProjectorException( "DependencyVisitor should not visit a formal world argument" );
	}
	...
	public Void visit( FormalWorldParameter n ) {
		throw new SoloistProjectorException( "DependencyVisitor should not visit a formal world parameter" );
	}
	...
	private List< WorldArgument > toWorldArguments( List< FormalWorldParameter > w ) {
		return w.stream().map( FormalWorldParameter::toWorldArgument ).collect( Collectors.toList() );
	}
	...
	private WorldArgument getWorldFromIndex( Node n, Integer i ) {
		...
		throw new SoloistProjectorException( "World '" + w.name().identifier() + "' not present in " + name );
	}
	...
	public static List< WorldArgument > getWorldArguments( List< CompilationUnit > compilationUnits, String template ) {
		List< WorldArgument > worlds =
				Streams.concat(
						compilationUnits.stream().map( CompilationUnit::interfaces ).flatMap( List::stream ),
						compilationUnits.stream().map( CompilationUnit::classes ).flatMap( List::stream ),
						compilationUnits.stream().map( CompilationUnit::enums ).flatMap( List::stream )
				).filter( t -> t.name().identifier().equals( template ) )
						.flatMap( t -> t.worldParameters().stream().map( FormalWorldParameter::toWorldArgument ) )
